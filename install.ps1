...
$UpdateBranch = if ($env:CODEX_ROUTER_UPDATE_BRANCH) {
  $env:CODEX_ROUTER_UPDATE_BRANCH
} else {
  "main"
}

function Normalize-RepositoryUrl([string]$Url) {
  return ([string]$Url).Trim().TrimEnd("/").ToLowerInvariant().Replace(".git", "")
}

function Ensure-ReferenceUpstream([string]$Directory) {
  $Existing = & git -C $Directory remote get-url upstream 2>$null
  if ($LASTEXITCODE -eq 0 -and $Existing) { return }
  & git -C $Directory remote add upstream $ReferenceUpstreamRepositoryUrl
  if ($LASTEXITCODE -ne 0) { throw "Unable to add the reference upstream remote." }
}
...
if (-not $CheckoutInstall) {
  ...
  if (Test-RouterCheckout $ScriptDirectory) {
    ...
    if (Test-Path (Join-Path $InstallDir ".git")) {
      ...
      ) | Where-Object { $_ } | ForEach-Object { Normalize-RepositoryUrl $_ }
      if ((Normalize-RepositoryUrl $Origin) -notin $AllowedOrigins) {
      ...
      if ($Branch -ne $UpdateBranch) {
        if (-not $Branch) {
          & git -C $InstallDir switch $UpdateBranch 2>$null
          if ($LASTEXITCODE -ne 0) {
            throw "$InstallDir is in a detached HEAD state and could not be restored to $UpdateBranch; run 'git switch $UpdateBranch' there and retry."
          }
          $Branch = & git -C $InstallDir branch --show-current
          if ($null -eq $Branch) { $Branch = "" }
          $Branch = [string]$Branch.Trim()
        }
        if ($Branch -ne $UpdateBranch) { throw "$InstallDir must be on its $UpdateBranch branch to update." }
      }
      # Running the enhanced installer is an explicit request to follow the
      # enhanced distribution. Do this only after the checkout has passed the
      # dirty/branch refusal gates so a refused update stays read-only.
      $ReferenceOrigins = @(
        $ReferenceUpstreamRepositoryUrl,
        "https://github.com/duolahypercho/codex-router",
        "git@github.com:duolahypercho/codex-router.git"
      ) | ForEach-Object { Normalize-RepositoryUrl $_ }
      if ((Normalize-RepositoryUrl $RepositoryUrl) -eq (Normalize-RepositoryUrl $DistributionRepositoryUrl) -and
          (Normalize-RepositoryUrl $Origin) -in $ReferenceOrigins) {
        & git -C $InstallDir remote set-url origin $DistributionRepositoryUrl
        if ($LASTEXITCODE -ne 0) { throw "Unable to repoint origin to the enhanced distribution." }
      }
      Ensure-ReferenceUpstream $InstallDir
      ...
      & git -C $InstallDir pull --ff-only origin $UpdateBranch
      ...
      & git clone --depth 1 --branch $UpdateBranch $RepositoryUrl $InstallDir
