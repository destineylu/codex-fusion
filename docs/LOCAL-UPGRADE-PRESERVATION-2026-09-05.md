...
> **最新升级接力请优先读取：`docs/LOCAL-UPGRADE-PRESERVATION-2026-09-18.md`。** 本文件继续保留 2026-09-03～09-05 的历史修复细节；2026-09-18 总档新增并覆盖了 ChatGPT Web、Codex Auto Resume、Control Center Tray 登录恢复、ComfyUI 0.3.0 双 target 等当前生产状态。
...
auto_compact_token_limit = 200,000
...
这是一条 exact-route payload workaround，不是把 Opus 5 context window 改成 200K，也不能无证据推广到其他 Xkiro 模型。

2026-09-08 又捕获到一次真实跨阈值失败：上一轮 Xkiro Opus 5 输入为 222,739 tokens、输出 13,837 tokens；由于旧 `auto_compact_token_limit = 230,000` 尚未在该轮输入前触发，下一轮序列化请求直接增长到 1,163,513 bytes，越过 1,150,000-byte 本地安全线。此前同一路由在约 229,937 输入 tokens 后能够正常自动 compact 到约 134,158，因此 compact 机制本身有效，问题是 230K 缺少单轮大输出的跳跃余量。当前阈值下调为 200K，保留约 30K tokens 的额外余量；1M context 声明与 1.15 MB 硬保护均保持不变。

### 8.1 Context Economy v1：第三方长任务成本保护

详细独立存档：`docs/LOCAL-PATCH-CONTEXT-ECONOMY-V1-2026-09-12.md`。发行版不得依赖维护者机器上的外部备份路径。

2026-09-12 根据真实用量审计增加。证据显示 Command Code DeepSeek / Muse 以及 Xkiro Opus 在长 Codex 任务中会把 15～30 万 token 的历史反复提交；其中 2026-09-06 DeepSeek V4 Flash 约 586 次请求、Router 估算输入约 170.7M tokens，2026-09-09 Muse Contributor 约 250 次请求、估算输入约 60.8M tokens。Opus 的 cache hit 很高，但 18～20 万级上下文仍增加套餐消耗、延迟和请求体风险。

实现边界：

```text
不改模型真实 context_window
不改 Light v2
不影响原生 GPT
不启用自动 failover / 不自动换模型
只对 exact-route 第三方昂贵编码模型降低 working auto-compact budget
```

当前策略：

```text
xkiro/anthropic/claude-opus-5                 -> 160K compact
commandcode-messages/claude-opus-5            -> 160K compact
commandcode-messages/claude-opus-4.8          -> 160K compact
commandcode/deepseek-v4-flash                  -> 160K compact
commandcode/deepseek-v4-pro                    -> 160K compact
commandcode/meta/muse-spark-1.3                -> 180K compact
commandcode/meta/muse-spark-1.3-contributor    -> 180K compact

soft pressure 约 100K
tool-result aging: >=16 KiB
recent frontier: 2
deferred Codex app tools: 默认不全量扩展，历史/forced choice 引用时按需补回
```
...
升级时若 upstream 已有等价的 per-route working-context budget / deferred tool surface，应 ADAPT 或 DROP 本地实现；不得把 160K 误改成模型真实 context window。关闭 Context Economy 后必须重新发布 catalog，使这些 route 恢复各自原始 auto-compact 值。
...
bundled app-server binary = %LOCALAPPDATA%\OpenAI\Codex\bin\<build-hash>\codex.exe
