...
$Commands = @(
  ...
  "smoke-test", "start", "stop", "test-model", "verify-model", "discover-models", "local-mlx",
...
function Open-ControlCenterWindow {
  ...
  # The canonical tray task is Interactive/Limited. Let the Windows supervisor
  # validate that exact task and issue the secondary launch under the same
  # Limited token, so an elevated installer can still reach Electron's
  # single-instance channel without elevating the tray primary itself.
  Invoke-RouterNode "src\tray-service.mjs" @("open")
}
...
function Restore-ControlCenterTaskSnapshot($TaskSnapshot) {
  ...
  $ActualSddl = Get-ControlCenterTaskSddl $TaskName
  if (-not (Test-SameControlCenterTaskSddl $ActualSddl ([string]$TaskSnapshot.Sddl))) {
...
function Test-SameControlCenterTaskSddl([string]$Left, [string]$Right) {
  try {
    ...
    if ([string]$LeftDescriptor.Owner -ne [string]$RightDescriptor.Owner -or
        [string]$LeftDescriptor.Group -ne [string]$RightDescriptor.Group) {
      return $false
    }

    # Task Scheduler canonicalizes a DACL when SetSecurityDescriptor writes it:
    # explicit ACEs move ahead of inherited ACEs and Windows may mark the ACL as
    # auto-inherited even when the effective ACE set is byte-for-byte identical.
    # Neither transformation changes the owner, group, permissions, inheritance
    # flags, or subjects. Compare every other control flag and the ACEs as a
    # multiset so rollback still fails closed on any actual permission change.
    $IgnoredFlags = [int][Security.AccessControl.ControlFlags]::DiscretionaryAclAutoInherited
    $LeftFlags = ([int]$LeftDescriptor.ControlFlags) -band (-bnot $IgnoredFlags)
    $RightFlags = ([int]$RightDescriptor.ControlFlags) -band (-bnot $IgnoredFlags)
    if ($LeftFlags -ne $RightFlags) { return $false }

    $LeftAcl = $LeftDescriptor.DiscretionaryAcl
    $RightAcl = $RightDescriptor.DiscretionaryAcl
    if (($null -eq $LeftAcl) -ne ($null -eq $RightAcl)) { return $false }
    if ($null -eq $LeftAcl) { return $true }
    if ($LeftAcl.Revision -ne $RightAcl.Revision -or $LeftAcl.Count -ne $RightAcl.Count) {
      return $false
    }

    function Get-AceFingerprints($Acl) {
      $Fingerprints = @()
      for ($Index = 0; $Index -lt $Acl.Count; $Index += 1) {
        $Ace = $Acl[$Index]
    ...
    $LeftAces = @(Get-AceFingerprints $LeftAcl)
    $RightAces = @(Get-AceFingerprints $RightAcl)
    for ($Index = 0; $Index -lt $LeftAces.Count; $Index += 1) {
...
switch ($Command) {
  ...
  "verify-model" { Invoke-RouterNode "src\verify-model.mjs" $Arguments }
