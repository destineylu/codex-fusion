...
test("install refuses before the tray has been built", () => {
  // Point the source root at a deterministic missing package so this remains
  // true even after a Windows developer has built release/win-unpacked while
  // debugging the real installer.
  const missingRoot = path.join(root, "generated", "missing-control-center-package-fixture");
  const result = trayService("install", { env: { CODEX_ROUTER_SOURCE_ROOT: missingRoot } });
...
test("graceful update drain is verified and lifecycle failure never authorizes a force-stop", () => {
  ...
  assert.match(drain, /requestControlCenterQuitForUpdate\(executable\)/);
  assert.match(drain, /exactUserExecutableOwnsPid\(executable, lifecycle\.pid\)/);
  // A stale lifecycle PID can be reused immediately by an unrelated process.
  // Ignore that mismatch only after the scheduler and an independent exact-exe
  // scan both prove there is no Control Center process left to drain.
  assert.match(drain, /if \(!exactUserExecutableOwnsPid\(executable, lifecycle\.pid\)\)[\s\S]*const schedulerState = schedulerOwned \? taskState\(\) : undefined/);
  assert.match(drain, /const exactProcesses = exactCanonicalProcessCount\(executable\)[\s\S]*if \(exactProcesses === 0 && schedulerState !== "running"\) return/);
  assert.match(drain, /reused Control Center lifecycle PID; it was not force-stopped/);
  ...
  assert.ok(
    drain.indexOf("exactUserExecutableOwnsPid(executable, lifecycle.pid)")
      < drain.indexOf("requestControlCenterQuitForUpdate(executable)"),
...
test("start restart and open require the exact canonical registered action", () => {
  ...
  assert.match(open, /tray-service\.mjs" @\("open"\)/);
  assert.doesNotMatch(open, /Start-Process|Stop-Process/);
  const supervisorOpen = source.slice(
    source.indexOf("function openControlCenterWindow("),
    source.indexOf("function requireBuiltTray("),
  );
  assert.match(supervisorOpen, /requireCanonicalRegisteredAction\(\)/);
  assert.match(supervisorOpen, /invokeControlCenterLimitedHelper\(registered\.execute, ""\)/);
  assert.match(supervisorOpen, /lifecycle\.pid === initial\.pid/);
  assert.match(supervisorOpen, /lifecycle\.visible/);
});
...
test("packaged GUI helpers preserve GUI mode and use one Limited current-user task", () => {
  const source = readFileSync(path.join(root, "src", "tray-service-windows.mjs"), "utf8");
  const wrapper = readFileSync(path.join(root, "codex-router.ps1"), "utf8");
  ...
  assert.match(guiEnvironment, /function invokeControlCenterLimitedHelper/);
  assert.match(guiEnvironment, /argument !== "" && argument !== "--quit-for-update"/);
  assert.match(guiEnvironment, /sameWindowsExecutable\(executable, TRAY_BINARY\)/);
  assert.match(guiEnvironment, /New-ScheduledTaskAction -Execute \$execute/);
  assert.match(guiEnvironment, /New-ScheduledTaskPrincipal -UserId \$principalName -LogonType Interactive -RunLevel Limited/);
  assert.match(guiEnvironment, /Register-ScheduledTask -TaskName \$taskName -Action \$action -Settings \$settings -Principal \$principal \| Out-Null/);
  assert.doesNotMatch(guiEnvironment, /Register-ScheduledTask[^\n]*-Force/);
  assert.match(guiEnvironment, /Start-ScheduledTask -TaskName \$taskName/);
  assert.match(guiEnvironment, /Unregister-ScheduledTask -TaskName \$taskName -Confirm:\$false -ErrorAction Stop/);
  assert.match(guiEnvironment, /windowsHide:\s*true/);
  assert.match(guiEnvironment, /requestControlCenterQuitForUpdate[\s\S]*invokeControlCenterLimitedHelper\(executable, "--quit-for-update"\)/);
  const query = source.slice(
    source.indexOf("function queryControlCenterLifecycle("),
    source.indexOf("function exactUserExecutableOwnsPid("),
  );
  ...
  assert.match(drain, /requestControlCenterQuitForUpdate\(executable\)/);
  assert.doesNotMatch(drain, /spawnSync\(executable, \["--quit-for-update"\]/);
  ...
  assert.match(open, /tray-service\.mjs" @\("open"\)/);
  assert.doesNotMatch(open, /Start-Process|ELECTRON_RUN_AS_NODE/);
});
...
test("Windows replacement recovery journals the package and exact prior Scheduled Task", () => {
  ...
  assert.match(transaction, /Test-SameControlCenterTaskSddl \$ActualSddl \(\[string\]\$TaskSnapshot\.Sddl\)/);
  ...
  const sddlComparison = script.slice(
    script.indexOf("function Test-SameControlCenterTaskSddl"),
    script.indexOf("function Read-ControlCenterTaskIdentityFromXml"),
  );
  assert.match(sddlComparison, /DiscretionaryAclAutoInherited/);
  assert.match(sddlComparison, /GetBinaryForm\(\$Bytes, 0\)/);
  assert.match(sddlComparison, /Sort-Object/);
  assert.match(sddlComparison, /LeftDescriptor\.Owner/);
  assert.match(sddlComparison, /LeftDescriptor\.Group/);
  assert.match(sddlComparison, /LeftAcl\.Revision -ne \$RightAcl\.Revision/);
  assert.doesNotMatch(sddlComparison, /GetSddlForm\(\$Sections\)/);
