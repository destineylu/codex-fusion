...
const XKIRO_OPUS5 = {
  ...
  description: "Local compaction payload-limit fixture.",
  priority: 600,
  defaultEffort: "high",
  ...
  compHash: "xkiro-opus5-payload-limit-fixture-v1",
};
...
test("enabled failover with no named chain never changes the selected model", async () => {
  const seen = [];
  const gw = await gateway(async (request, response) => {
    const body = await bodyJson(request);
    seen.push(body);
  ...
  const routerPort = await openPort();
  const child = run(routerEnv(gw.port, routerPort), { enabled: true, chain: [] });
  try {
    await waitFor(`http://127.0.0.1:${routerPort}/health`, child);
    const result = await readRouted(routerPort, TURN_BODY);
...
test("oversized Xkiro Opus 5 compaction stays on the selected model without explicit fallback consent", async () => {
  const seen = [];
  const gw = await gateway(async (request, response) => {
    seen.push(await bodyJson(request));
    response.writeHead(500, { "Content-Type": "application/json" });
    response.end(JSON.stringify({ error: { message: "must not be reached" } }));
  });
  const routerPort = await openPort();
  const child = run(routerEnv(gw.port, routerPort), {
    enabled: true,
    chain: [],
    userModels: [XKIRO_OPUS5],
  });
  try {
    await waitFor(`http://127.0.0.1:${routerPort}/health`, child);
    const result = await readRouted(
      routerPort,
      {
        model: XKIRO_OPUS5.slug,
        input: [
          {
            type: "message",
            role: "user",
            content: [{ type: "input_text", text: "x".repeat(1_160_000) }],
          },
        ],
      },
    ...
    assert.equal(seen.length, 0, "no alternate model may be contacted without an explicit chain");
...
test("Xkiro compaction absorbs duplicate-request 409s on the selected model without failover", async () => {
  const seen = [];
  let calls = 0;
  const duplicateBody = JSON.stringify({
    error: {
      message:
        "litellm.APIError: APIError: OpenAIException - A duplicate request is already being processed — please try again.",
      type: "server_error",
      code: "internal_error",
    },
  });
  const gw = await gateway(async (request, response) => {
    ...
    calls += 1;
    if (calls <= 2) {
      const bytes = Buffer.from(duplicateBody, "utf8");
    ...
    const payload = {
      id: "resp-xkiro-compact",
      object: "response",
      output: [
        {
          type: "message",
          role: "assistant",
          content: [{ type: "output_text", text: "xkiro compact summary" }],
        },
      ],
      usage: { input_tokens: 1_000, output_tokens: 12, total_tokens: 1_012 },
    };
  ...
  const child = run(
    {
      ...
      CODEX_ROUTER_XKIRO_COMPACTION_DUPLICATE_RETRY_DELAYS_MS: "1,1,1",
    },
    {
      enabled: false,
      chain: [],
  ...
  try {
    ...
    assert.equal(seen.every((body) => body.model === XKIRO_OPUS5.gatewayModel), true);
    ...
    assert.equal(events[0].model, XKIRO_OPUS5.slug);
...
test("oversized Xkiro Opus 5 compaction skips the unsafe route and uses the configured fallback", async () => {
  const seen = [];
  const gw = await gateway(async (request, response) => {
    const body = await bodyJson(request);
    seen.push(body);
    assert.equal(
      ...
      "the oversized Xkiro compaction reached the gateway instead of being skipped locally",
    );
    const payload = {
      id: "resp-compact-fallback",
      object: "response",
      output: [
        {
          type: "message",
          role: "assistant",
          content: [{ type: "output_text", text: "compact summary from fallback" }],
        },
      ],
      usage: { input_tokens: 360_000, output_tokens: 12, total_tokens: 360_012 },
    };
    const bytes = Buffer.from(JSON.stringify(payload), "utf8");
    response.writeHead(200, {
      "Content-Type": "application/json",
  ...
  try {
    ...
    assert.equal(seen.length, 1, "the skipped Xkiro route must not count as an upstream attempt");
    ...
    assert.equal(events[0].status, 200);
    assert.equal(events[0].failoverFrom, XKIRO_OPUS5.slug);
    assert.match(child.testErrors(), /reason=compaction\/provider_payload_limit/);
    assert.doesNotMatch(child.testErrors(), /model=xkiro\/anthropic\/claude-opus-5 provider=xkiro status=/);
  } finally {
    await stopChild(child);
    await closeServer(gw.server);
  }
