...
test("already-flat Codex collaboration functions retain native restore identity", () => {
  const spawnSchema = {
    type: "object",
    properties: {
      model: {
        type: "string",
        enum: ["commandcode/deepseek-v4-flash", "commandcode/gpt-5.6-luna"],
      },
    },
  ...
  const { tools, flattened, namespaces } = flattenNamespaceTools([
    ...
    {
      type: "function",
      name: "collaboration__literal_user_tool",
      parameters: { type: "object", properties: {}, additionalProperties: false },
    },
  ]);

  assert.equal(flattened, true, "the already-flat built-ins are a semantic namespace relay");
  assert.deepEqual(
    tools.map((tool) => tool.name),
    [
      "collaboration__spawn_agent",
      "collaboration__list_agents",
      "collaboration__literal_user_tool",
    ],
    "provider-visible names stay byte-for-byte compatible",
  );
  assert.deepEqual(
    [...namespaces.get("collaboration")].sort(),
    ["list_agents", "spawn_agent"],
    "only known Codex collaboration built-ins are adopted",
  );

  const lookups = buildNamespaceLookups(namespaces);
  const spawn = rewriteNamespaceFunctionCall(
    {
      item: {
        type: "function_call",
        name: "collaboration__spawn_agent",
        call_id: "call_spawn",
        arguments: JSON.stringify({ model: "commandcode/gpt-5.6-luna" }),
      },
    },
    lookups,
    "xkiro/anthropic/claude-opus-5",
  );
  assert.equal(spawn.item.name, "spawn_agent");
  assert.equal(spawn.item.namespace, "collaboration");
  assert.deepEqual(JSON.parse(spawn.item.arguments), { model: "commandcode/gpt-5.6-luna" });

  const list = rewriteNamespaceFunctionCall(
    {
      item: {
        type: "function_call",
        name: "collaboration__list_agents",
  ...
  assert.equal(list.item.name, "list_agents");
  assert.equal(list.item.namespace, "collaboration");

  const literal = rewriteNamespaceFunctionCall(
    {
      item: {
        type: "function_call",
        name: "collaboration__literal_user_tool",
        call_id: "call_literal",
        arguments: "{}",
      },
    },
    lookups,
  ...
  assert.equal(literal, undefined, "unknown prefixed functions remain ordinary functions");
});

test("explicit function namespaces from Codex Desktop round-trip through chat providers", () => {
  const spawnSchema = {
    type: "object",
    properties: {
      model: {
  ...
  assert.deepEqual(
    tools.map((tool) => tool.name),
    ["collaboration__spawn_agent", "codex_app__list_projects", "plain_tool"],
  );
  assert.equal(tools[0].namespace, undefined, "provider copy must not retain native namespace");
  assert.equal(tools[1].namespace, undefined, "provider copy must not retain native namespace");
  assert.deepEqual(tools[0].parameters, spawnSchema);
  assert.deepEqual([...namespaces.get("collaboration")], ["spawn_agent"]);
  assert.deepEqual([...namespaces.get("codex_app")], ["list_projects"]);
  ...
  const spawn = rewriteNamespaceFunctionCall(
    {
      item: {
        type: "function_call",
        name: "collaboration__spawn_agent",
        call_id: "call_spawn_explicit",
        arguments: JSON.stringify({ model: "commandcode/deepseek-v4-flash" }),
      },
    },
    lookups,
    "xkiro/anthropic/claude-opus-5",
  ...
  const app = rewriteNamespaceFunctionCall(
    {
      item: {
        type: "function_call",
        name: "codex_app__list_projects",
        call_id: "call_app_explicit",
        arguments: "{}",
      },
    },
    lookups,
  ...
  assert.equal(app.item.name, "list_projects");
  assert.equal(app.item.namespace, "codex_app");
});

test("routed collaboration message tools advertise plaintext without mutating the client schema", () => {
  const spawnSchema = {
    type: "object",
    properties: {
  ...
  const { tools } = flattenNamespaceTools([original]);
  const routed = tools[0];
  assert.equal(routed.name, "collaboration__spawn_agent");
  assert.equal(routed.parameters.properties.message.encrypted, undefined);
  assert.equal(routed.inputSchema.properties.message.encrypted, undefined);
  assert.match(
    routed.parameters.properties.message.description,
    /routed third-party parents.*ordinary plaintext/i,
  );
  assert.equal(
    spawnSchema.properties.message.encrypted,
    true,
    "the Codex-owned client schema must remain untouched",
  );
  assert.equal(
    original.tools[0].inputSchema.properties.message.encrypted,
    true,
    "the native collaboration definition stays encrypted for the client",
  );
});

test("already-flat routed collaboration definitions drop only the encrypted message marker", () => {
  const schema = {
    type: "object",
    properties: {
      target: { type: "string" },
  ...
  assert.equal(tools[0].name, "collaboration__send_message");
  assert.equal(tools[0].parameters.properties.message.encrypted, undefined);
  assert.equal(schema.properties.message.encrypted, true);
  assert.deepEqual(tools[0].parameters.required, ["target", "message"]);
});

test("routed collaboration history replaces native ciphertext without losing task metadata", () => {
  ...
  const encryptedMessage = `gAAAAA${"A".repeat(80)}`;
  const history = [
    {
      type: "function_call",
      name: "spawn_agent",
      namespace: "collaboration",
      call_id: "call_old_spawn",
      encrypted_function_args: ["message"],
      arguments: JSON.stringify({
        task_name: "old_worker",
        agent_type: "router_commandcode_deepseek_v4_flash",
        message: encryptedMessage,
      }),
    },
  ];

  const routed = flattenNamespacedHistory(history, namespaces);
  assert.equal(routed[0].name, "collaboration__spawn_agent");
  assert.equal(routed[0].namespace, undefined);
  assert.equal(routed[0].encrypted_function_args, undefined);
  const args = JSON.parse(routed[0].arguments);
  assert.equal(args.task_name, "old_worker");
  assert.equal(args.agent_type, "router_commandcode_deepseek_v4_flash");
  assert.match(args.message, /Historical encrypted collaboration payload omitted/i);
  assert.doesNotMatch(args.message, /^gAAAAA/);
  assert.equal(
    JSON.parse(history[0].arguments).message,
    encryptedMessage,
    "stored native history must not be mutated",
  );
});

test("plaintext routed collaboration calls opt into Codex DirectPlaintextMessage delivery", () => {
  const { namespaces } = flattenNamespaceTools([
    {
      type: "namespace",
      name: "collaboration",
  ...
  for (const [name, args] of [
    ["spawn_agent", { task_name: "worker", message: "Inspect the PDF filename behavior." }],
    ["send_message", { target: "/root/worker", message: "Continue with the regression check." }],
    ["followup_task", { target: "/root/worker", message: "Verify the final filename now." }],
  ]) {
    const rewritten = rewriteNamespaceFunctionCall(
      {
        type: "response.output_item.done",
        item: {
          type: "function_call",
          name: `collaboration__${name}`,
          call_id: `call_plain_${name}`,
    ...
    assert.equal(rewritten.item.name, name);
    assert.equal(rewritten.item.namespace, "collaboration");
    assert.deepEqual(rewritten.item.encrypted_function_args, []);
    assert.deepEqual(JSON.parse(rewritten.item.arguments), args);
  }
});

test("native ciphertext from a routed parent is never mislabeled as plaintext", () => {
  const { namespaces } = flattenNamespaceTools([
    {
      type: "namespace",
      name: "collaboration",
  ...
  const lookups = buildNamespaceLookups(namespaces);
  const encryptedMessage = `gAAAAA${"B".repeat(80)}`;
  const rewritten = rewriteNamespaceFunctionCall(
    {
      type: "response.output_item.done",
      item: {
        type: "function_call",
        name: "collaboration__spawn_agent",
        call_id: "call_ciphertext_spawn",
        encrypted_function_args: [],
        arguments: JSON.stringify({ message: encryptedMessage }),
      },
    },
    lookups,
    "xkiro/anthropic/claude-opus-5",
  ...
  assert.equal(rewritten.item.name, "spawn_agent");
  assert.equal(rewritten.item.namespace, "collaboration");
  assert.equal(rewritten.item.encrypted_function_args, undefined);
  assert.equal(JSON.parse(rewritten.item.arguments).message, encryptedMessage);
});
...
test("streamed plaintext collaboration closes carry the Codex direct-plaintext marker", async () => {
  const { namespaces } = flattenNamespaceTools([
    {
      type: "namespace",
      name: "collaboration",
  ...
  const args = JSON.stringify({
    task_name: "ai1_plaintext_stream",
    message: "Verify the fixed PDF filename and report the result.",
  });
  const events = [
    {
      type: "response.output_item.added",
  ...
  const output = await collect(
    Readable.from(events).pipe(new NamespaceToolCallTransform(namespaces)),
  );
  const payloads = output
    .split(/\n\n/)
    .filter(Boolean)
    .map((frame) => JSON.parse(frame.replace(/^data: /, "")));
  const added = payloads.find((event) => event.type === "response.output_item.added");
  const done = payloads.find((event) => event.type === "response.output_item.done");
  ...
  assert.equal(added.item.encrypted_function_args, undefined);
...
test("response transform detects SSE even when a gateway mislabels it as JSON", async () => {
  const { namespaces } = flattenNamespaceTools(clientRoutedTools());
  const body = Buffer.from(
    `data: ${JSON.stringify({
      type: "response.output_item.done",
      item: {
        type: "function_call",
        name: "collaboration__spawn_agent",
        call_id: "call_mislabeled_json_sse",
        arguments: "{}",
      },
    })}\n\n`,
    "utf8",
  ...
  const output = await collect(Readable.from([body]).pipe(transform));
  assert.match(output, /"name":"spawn_agent"/);
  assert.match(output, /"namespace":"collaboration"/);
  assert.doesNotMatch(output, /collaboration__spawn_agent/);
});

test("large LiteLLM response prelude does not disable later namespace restoration", async () => {
  const { namespaces } = flattenNamespaceTools(clientRoutedTools());
  const created = {
    type: "response.created",
    response: {
      id: "resp_large_tool_catalog",
      object: "response",
      status: "in_progress",
      output: [],
      tools: [
  ...
  const spawned = {
    ...
    item: {
      type: "function_call",
      id: "call_after_large_prelude",
      call_id: "call_after_large_prelude",
      name: "collaboration__spawn_agent",
      arguments: "{}",
      status: "completed",
    },
  ...
  const body = [
    ...
    `event: response.output_item.done\ndata: ${JSON.stringify(spawned)}\n\n`,
  ].join("");
  assert.ok(Buffer.byteLength(body) > 256 * 1024, "fixture must cross the old frame limit");

  const output = await collect(
    Readable.from([Buffer.from(body)]).pipe(
