...
test("shared-config discovery includes direct and PATH Codex binaries without duplicates", () => {
  const root = mkdtempSync(path.join(os.tmpdir(), "codex-router-shared-config-binaries-"));
  const desktop = path.join(root, "desktop", process.platform === "win32" ? "codex.exe" : "codex");
  const cli = path.join(root, "cli", process.platform === "win32" ? "codex.cmd" : "codex");
  ...
  try {
    assert.deepEqual(
      codexBinariesForSharedConfig({
        explicitBinary: null,
        candidatePaths: [desktop, desktop],
        pathBinary: cli,
        resolvedBinary: desktop,
      }),
      [path.resolve(desktop), path.resolve(cli)],
    );
    assert.deepEqual(
      codexBinariesForSharedConfig({
        explicitBinary: cli,
        candidatePaths: [desktop],
        pathBinary: desktop,
        resolvedBinary: desktop,
      }),
      [path.resolve(cli)],
...
test("shared-config discovery ignores Windows extensionless npm POSIX shims", { skip: process.platform !== "win32" }, () => {
  const root = mkdtempSync(path.join(os.tmpdir(), "codex-router-extensionless-shim-"));
  const exe = path.join(root, "desktop", "codex.exe");
  const cmd = path.join(root, "npm", "codex.cmd");
  const posixShim = path.join(root, "npm", "codex");
  mkdirSync(path.dirname(exe), { recursive: true });
  mkdirSync(path.dirname(cmd), { recursive: true });
  writeFileSync(exe, "");
  writeFileSync(cmd, "");
  ...
  try {
    assert.deepEqual(
      codexBinariesForSharedConfig({
        ...
        candidatePaths: [exe],
        pathBinary: cmd,
        resolvedBinary: posixShim,
      }),
      [path.resolve(exe), path.resolve(cmd)],
    );
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
