...
test("Command Code aliases over-64-character tools and restores their native identity", () => {
  const longName = `mcp__commandcode_regression__${"long_tool_segment_".repeat(4)}`;
  assert.ok(longName.length > 64, "fixture must reproduce Command Code's rejected name length");
  const client = [
    {
      type: "function",
      name: longName,
      parameters: {
        type: "object",
        properties: { unique_commandcode_probe: { type: "string" } },
      },
    },
  ];

  const routed = chatProviderToolSurface(client, "commandcode");
  const aliased = routed.tools.find(
    (tool) => tool.parameters?.properties?.unique_commandcode_probe,
  );
  assert.ok(aliased);
  assert.equal(aliased.name.length, 64);
  assert.notEqual(aliased.name, longName);

  const history = flattenNamespacedHistory(
    [{ type: "function_call", name: longName, call_id: "long-1", arguments: "{}" }],
    routed.namespaces,
  );
  assert.equal(history[0].name, aliased.name);
  assert.deepEqual(
    flattenToolChoice({ type: "function", name: longName }, routed.namespaces),
    { type: "function", name: aliased.name },
  );

  const restored = rewriteNamespaceResponsePayload(
    {
      output: [
        { type: "function_call", name: aliased.name, call_id: "long-2", arguments: "{}" },
      ],
    },
    buildNamespaceLookups(routed.namespaces),
  );
  assert.equal(restored.output[0].name, longName);
  assert.equal(restored.output[0].namespace, undefined);

  const unbounded = chatProviderToolSurface(client, "openrouter");
  const unchanged = unbounded.tools.find(
    (tool) => tool.parameters?.properties?.unique_commandcode_probe,
  );
  assert.equal(unchanged.name, longName, "the 64-character cap stays Command Code-specific");
});
