...
// Simulates the older v2 schema: the feature exists, but only the legacy map
// with `enabled` and `expose_spawn_agent_model_overrides` is accepted.
const legacyV2Codex = (() => {
  const isWindows = process.platform === "win32";
  const file = path.join(
    codexStubDir,
    isWindows ? "codex-legacy-v2.cmd" : "codex-legacy-v2",
  );
  const contents = isWindows
    ? `@echo off\r\nfindstr /c:"multi_agent_v2 = { enabled = true" "%CODEX_HOME%\\config.toml" >nul 2>&1\r\nif %errorlevel% equ 0 (\r\n  echo Not logged in 1>&2\r\n  exit /b 1\r\n)\r\nfindstr /c:"multi_agent_v2" "%CODEX_HOME%\\config.toml" >nul 2>&1\r\nif %errorlevel% equ 0 (\r\n  echo Error loading configuration: legacy multi_agent_v2 schema required 1>&2\r\n  exit /b 1\r\n)\r\necho Not logged in 1>&2\r\nexit /b 1\r\n`
    : `#!/bin/sh
if grep -q 'multi_agent_v2 = { enabled = true' "$CODEX_HOME/config.toml" 2>/dev/null; then
  echo 'Not logged in' >&2
  exit 1
fi
if grep -q multi_agent_v2 "$CODEX_HOME/config.toml" 2>/dev/null; then
  echo 'Error loading configuration: legacy multi_agent_v2 schema required' >&2
  exit 1
fi
echo 'Not logged in' >&2
exit 1
...
// Codex 0.144.x types features.multi_agent_v2 as a boolean. Structured maps
// fail config loading, but `multi_agent_v2 = true` is valid and still needs the
// older standalone concurrency scalar.
const booleanV2Codex = (() => {
  const isWindows = process.platform === "win32";
  const file = path.join(
    codexStubDir,
    isWindows ? "codex-boolean-v2.cmd" : "codex-boolean-v2",
  );
  const contents = isWindows
    ? `@echo off\r\nfindstr /c:"multi_agent_v2 = {" "%CODEX_HOME%\\config.toml" >nul 2>&1\r\nif %errorlevel% equ 0 (\r\n  echo Error loading configuration: invalid type: map, expected a boolean 1>&2\r\n  exit /b 1\r\n)\r\nif "%~1"=="features" if "%~2"=="list" (\r\n  findstr /c:"multi_agent_v2 = true" "%CODEX_HOME%\\config.toml" >nul 2>&1\r\n  if %errorlevel% equ 0 echo multi_agent_v2                       under development  true\r\n  exit /b 0\r\n)\r\necho Not logged in 1>&2\r\nexit /b 1\r\n`
    : `#!/bin/sh
if grep -q 'multi_agent_v2 = {' "$CODEX_HOME/config.toml" 2>/dev/null; then
...
if [ "$1" = features ] && [ "$2" = list ]; then
  if grep -q 'multi_agent_v2 = true' "$CODEX_HOME/config.toml" 2>/dev/null; then
    echo 'multi_agent_v2                       under development  true'
  fi
  exit 0
fi
echo 'Not logged in' >&2
...
test("router mode parks root context overrides and restores them only on disable", () => {
  const codexHome = mkdtempSync(path.join(os.tmpdir(), "codex-router-context-overrides-"));
  const stateDir = path.join(codexHome, "router-state");
  const configPath = path.join(codexHome, "config.toml");
  const statePath = path.join(stateDir, "router-context-overrides.json");
  const original = `model = "gpt-5.6-sol"
model_provider = "openai"
model_context_window = 1000000 # user native override
...
`;
  ...
  try {
    ...
    assert.equal(enabled.mode, "router");
    assert.equal(enabled.router_context_overrides_parked, true);
    assert.equal(enabled.router_context_override_count, 3);
    const routed = readFileSync(configPath, "utf8");
    assert.doesNotMatch(routed, /^model_context_window\s*=/m);
    assert.doesNotMatch(routed, /^model_auto_compact_token_limit\s*=/m);
    assert.doesNotMatch(routed, /^model_auto_compact_token_limit_scope\s*=/m);
    ...
    assert.equal(reenabled.router_context_overrides_parked, true);
    assert.equal(reenabled.router_context_override_count, 3);

    const disabled = run("disable", codexHome, stateDir);
    assert.equal(disabled.mode, "native");
    const restored = readFileSync(configPath, "utf8");
    assert.match(restored, /^model_context_window = 1000000 # user native override$/m);
    assert.match(restored, /^model_auto_compact_token_limit = 900000$/m);
    assert.match(restored, /^model_auto_compact_token_limit_scope = "total"$/m);
...
test("login-free toggles keep Router-wide context overrides parked", () => {
  const codexHome = mkdtempSync(path.join(os.tmpdir(), "codex-router-context-login-free-"));
  const stateDir = path.join(codexHome, "router-state");
  const configPath = path.join(codexHome, "config.toml");
  writeFileSync(
    configPath,
...
`,
  ...
  try {
    ...
    assert.equal(loginFree.login_free, true);
    assert.equal(loginFree.router_context_overrides_parked, true);
    assert.doesNotMatch(readFileSync(configPath, "utf8"), /^model_context_window\s*=/m);
    assert.doesNotMatch(readFileSync(configPath, "utf8"), /^model_auto_compact_token_limit\s*=/m);

    const signedInRouter = run("login-free-disable", codexHome, stateDir);
    assert.equal(signedInRouter.mode, "router");
    assert.equal(signedInRouter.login_free, false);
    assert.equal(signedInRouter.router_context_overrides_parked, true);
    assert.doesNotMatch(readFileSync(configPath, "utf8"), /^model_context_window\s*=/m);
    assert.doesNotMatch(readFileSync(configPath, "utf8"), /^model_auto_compact_token_limit\s*=/m);

    run("disable", codexHome, stateDir);
    const native = readFileSync(configPath, "utf8");
    assert.match(native, /^model_context_window = 1000000$/m);
    assert.match(native, /^model_auto_compact_token_limit = 900000$/m);
  } finally {
    rmSync(codexHome, { recursive: true, force: true });
  }
});
...
test("config manager falls back to the legacy multi_agent_v2 map on older v2 builds", () => {
  const codexHome = mkdtempSync(path.join(os.tmpdir(), "codex-router-v2-legacy-shape-"));
  const configPath = path.join(codexHome, "config.toml");
  writeFileSync(configPath, `model = "gpt-5.5"\n`, { mode: 0o600 });

  try {
    run("enable", codexHome, undefined, [], { CODEX_BIN: legacyV2Codex });
    const enabled = readFileSync(configPath, "utf8");
    assert.match(enabled, /# BEGIN codex-router-multi-agent-v2-managed/);
    assert.match(
      enabled,
...
test("config manager falls back to the boolean multi_agent_v2 gate on Codex 0.144-style builds", () => {
  const codexHome = mkdtempSync(path.join(os.tmpdir(), "codex-router-v2-boolean-shape-"));
  const configPath = path.join(codexHome, "config.toml");
  writeFileSync(configPath, `model = "gpt-5.5"\n`, { mode: 0o600 });

  try {
    run("enable", codexHome, undefined, [], { CODEX_BIN: booleanV2Codex });
    const enabled = readFileSync(configPath, "utf8");
    assert.match(enabled, /# BEGIN codex-router-multi-agent-v2-managed/);
    assert.match(enabled, /^multi_agent_v2 = true$/m);
    assert.doesNotMatch(enabled, /multi_agent_v2 = \{/);
    assert.match(enabled, /# BEGIN codex-router-agent-concurrency-managed/);
    assert.match(enabled, /^max_concurrent_threads_per_session = 6$/m);
  } finally {
    rmSync(codexHome, { recursive: true, force: true });
  }
...
test("config manager migrates structured context management to the shared boolean feature schema", () => {
  const codexHome = mkdtempSync(path.join(os.tmpdir(), "codex-router-context-management-shared-"));
  const configPath = path.join(codexHome, "config.toml");
  writeFileSync(
    configPath,
    `model = "gpt-5.5"
...
# BEGIN codex-context-light-v2-managed
# LIGHT v2 fixture preamble
[apps.canva]
enabled = false

[[skills.config]]
path = "C:/skills/specialized/SKILL.md"
enabled = false
# END codex-context-light-v2-managed
...
`,
  ...
  try {
    run("enable", codexHome, undefined, [], { CODEX_BIN: booleanV2Codex });
    const enabled = readFileSync(configPath, "utf8");
    assert.doesNotMatch(enabled, /^\[features\.context_management\]$/m);
    assert.match(enabled, /^\[features\]$/m);
    assert.match(enabled, /^context_management = true$/m);
    assert.match(enabled, /^multi_agent_v2 = true$/m);
    assert.equal(enabled.includes("[projects.'C:\\work']"), true);
    assert.match(enabled, /^trust_level = "trusted"$/m);
    const multiAgent = enabled.indexOf("# BEGIN codex-router-multi-agent-v2-managed");
    const lightStart = enabled.indexOf("# BEGIN codex-context-light-v2-managed");
    const canva = enabled.indexOf("[apps.canva]");
    const lightEnd = enabled.indexOf("# END codex-context-light-v2-managed");
    const project = enabled.indexOf("[projects.'C:\\work']");
    assert.ok(multiAgent !== -1 && multiAgent < lightStart);
    assert.ok(lightStart !== -1 && lightStart < canva);
    assert.ok(canva < lightEnd);
    assert.ok(lightEnd < project);
  } finally {
    rmSync(codexHome, { recursive: true, force: true });
  }
});

test("config manager repairs a hoisted LIGHT v2 preamble without capturing user tables", () => {
  const codexHome = mkdtempSync(path.join(os.tmpdir(), "codex-router-light-marker-repair-"));
  const configPath = path.join(codexHome, "config.toml");
  writeFileSync(
    configPath,
    `model = "gpt-5.5"
...
`,
  ...
  try {
    ...
    const enabled = readFileSync(configPath, "utf8");
    const lightStart = enabled.indexOf("# BEGIN codex-context-light-v2-managed");
    const windows = enabled.indexOf("[windows]");
    const project = enabled.indexOf("[projects.'C:\\work']");
    const canva = enabled.indexOf("[apps.canva]");
    const lightEnd = enabled.indexOf("# END codex-context-light-v2-managed");
    assert.ok(windows !== -1 && windows < lightStart);
    assert.ok(project !== -1 && project < lightStart);
    assert.ok(lightStart < canva && canva < lightEnd);
    assert.ok(enabled.indexOf("# BEGIN codex-router-multi-agent-v2-managed") < lightStart);
  } finally {
    rmSync(codexHome, { recursive: true, force: true });
  }
});
...
model_reasoning_effort = "high"
model_context_window = 1000000 # user-wide native override
model_auto_compact_token_limit = 900000
model_auto_compact_token_limit_scope = "total"
...
`,
  ...
  try {
    ...
    assert.doesNotMatch(loginFreeConfig, /^model_context_window\s*=/m);
    assert.doesNotMatch(loginFreeConfig, /^model_auto_compact_token_limit\s*=/m);
    assert.doesNotMatch(loginFreeConfig, /^model_auto_compact_token_limit_scope\s*=/m);
    ...
    assert.deepEqual(originalState.previousContextOverrides, [
      "model_context_window = 1000000 # user-wide native override",
      "model_auto_compact_token_limit = 900000",
      'model_auto_compact_token_limit_scope = "total"',
    ]);
    ...
    assert.match(readFileSync(configPath, "utf8"), /^model_context_window = 1000000 # user-wide native override$/m);
    assert.match(readFileSync(configPath, "utf8"), /^model_auto_compact_token_limit = 900000$/m);
    assert.match(readFileSync(configPath, "utf8"), /^model_auto_compact_token_limit_scope = "total"$/m);
...
test("login-free refresh migrates a v1 snapshot and parks global context overrides", () => {
  const codexHome = mkdtempSync(path.join(os.tmpdir(), "codex-router-login-free-v1-context-"));
  const stateDir = path.join(codexHome, "router-state");
  const configPath = path.join(codexHome, "config.toml");
  const providerModePath = path.join(stateDir, "codex-provider-mode.json");
  writeFileSync(
...
`,
  ...
  try {
    run("enable", codexHome, stateDir);
    const legacyActive = readFileSync(configPath, "utf8")
      .replace(/^model = ".*"$/m, 'model = "xkiro/anthropic/claude-opus-5"')
      .replace(/^model_provider = ".*"$/m, 'model_provider = "codex-router"');
    writeFileSync(configPath, legacyActive, { mode: 0o600 });
    writeFileSync(
      providerModePath,
    ...
    assert.equal(refreshed.login_free, true);
    assert.equal(refreshed.login_free_context_overrides_parked, true);
    assert.equal(refreshed.login_free_context_override_count, 2);
    const routed = readFileSync(configPath, "utf8");
    assert.doesNotMatch(routed, /^model_context_window\s*=/m);
    assert.doesNotMatch(routed, /^model_auto_compact_token_limit\s*=/m);
    const migratedState = JSON.parse(readFileSync(providerModePath, "utf8"));
    assert.equal(migratedState.version, 1);
    assert.deepEqual(migratedState.previousContextOverrides, [
      "model_context_window = 1000000",
      "model_auto_compact_token_limit = 900000",
    ]);
    ...
    assert.match(native, /^model_context_window = 1000000$/m);
    assert.match(native, /^model_auto_compact_token_limit = 900000$/m);
    assert.match(native, /^model = "gpt-5.6-sol"$/m);
    assert.match(native, /^model_provider = "openai"$/m);
  } finally {
    rmSync(codexHome, { recursive: true, force: true });
  }
});
