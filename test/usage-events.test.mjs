...
test("Command Code usage events snapshot request pricing into the append-only ledger", async () => {
  const stateDir = mkdtempSync(path.join(os.tmpdir(), "model-router-commandcode-usage-"));
  const previousStateDir = process.env.MODEL_ROUTER_STATE_DIR;
  process.env.MODEL_ROUTER_STATE_DIR = stateDir;
  let usage;
  try {
    usage = await import(`../src/usage-events.mjs?commandcode=${Date.now()}`);
    usage.recordUsageEvent({
    ...
    const [event] = usage.allUsageEvents();
    assert.equal(event.commandCodeBilling.pricingVersion, "goat-2026-09-06");
    assert.equal(event.commandCodeBilling.modelLabel, "Muse Spark 1.3 Contributor");
    assert.equal(event.commandCodeBilling.complete, true);
    assert.equal(event.commandCodeBilling.retrospective, undefined);
    assert.equal(event.commandCodeBilling.goatMultiplier, 3.5);
    assert.ok(event.commandCodeBilling.usageValueUsd > 0);
  } finally {
    if (previousStateDir === undefined) delete process.env.MODEL_ROUTER_STATE_DIR;
    else process.env.MODEL_ROUTER_STATE_DIR = previousStateDir;
    rmSync(stateDir, { recursive: true, force: true });
