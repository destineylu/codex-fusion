...
test("captures cache-write token counts when a provider reports them", () => {
  assert.deepEqual(
    normalizeTokenUsage({
      input_tokens: 100,
      output_tokens: 5,
      input_tokens_details: { cached_tokens: 70, cache_write_tokens: 12 },
    }),
    {
      inputTokens: 100,
      outputTokens: 5,
...
test("adds up the usage of two attempts at one turn", () => {
  ...
  assert.deepEqual(
    mergeTokenUsage(
      { inputTokens: 10, outputTokens: 1, totalTokens: 11, cacheWriteInputTokens: 2 },
      { inputTokens: 10, outputTokens: 1, totalTokens: 11, cachedInputTokens: 8, cacheWriteInputTokens: 3 },
    ),
    {
      inputTokens: 20,
      outputTokens: 2,
