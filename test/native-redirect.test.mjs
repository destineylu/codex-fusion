...
test("reserve redirect reads only the persisted root model selection", () => {
  ...
  assert.equal(readReserveRedirectTarget(configPath), "commandcode/deepseek-v4-flash");

  writeFileSync(configPath, 'model = "gpt-reserve"\n', "utf8");
  assert.equal(readReserveRedirectTarget(configPath), undefined);
  assert.equal(readReserveRedirectTarget(path.join(stateDir, "missing.toml")), undefined);
});
