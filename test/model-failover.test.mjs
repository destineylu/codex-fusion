...
test("failover is off until the operator explicitly authorizes fallback models", () => {
  const settings = readFailoverSettings();
  assert.equal(settings.enabled, false);
  assert.equal(settings.defaulted, true);
  assert.equal(failoverAuthorized(settings), false);
});

test("failover requires both the switch and an explicit named chain", () => {
  ...
  assert.equal(failoverAuthorized(), false);

  setFailoverEnabled(true);
  assert.equal(readFailoverSettings().enabled, true);
  assert.equal(failoverAuthorized(), false);

  setFailoverChain(["zai-api/glm-5.2"]);
  assert.equal(failoverAuthorized(), true);
