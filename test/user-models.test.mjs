...
test("self-prefixed upstream ids keep one public provider namespace", () => {
  const entry = userModelEntry({
    providerId: "chatgpt-web",
    upstreamId: "chatgpt-web/light",
    priority: 100,
  });
  assert.equal(entry.slug, "chatgpt-web/light");
  assert.equal(entry.gatewayModel, "chatgpt-web-light");
  assert.equal(entry.upstreamModel, "chatgpt-web/light");
});
...
test("registry merges valid user models and skips collisions", async () => {
  const entries = [
    ...
    // Untouched curated names are presented with the provider that actually
    // serves them, while protocol variants collapse to their canonical account.
    userModelEntry({
      providerId: "xkiro",
      upstreamId: "anthropic/provider-label-test",
      priority: 116,
    }),
    userModelEntry({
      providerId: "commandcode-messages",
      upstreamId: "anthropic/messages-provider-label-test",
      priority: 117,
    }),
    // An operator-authored display name is not a default and must remain exact.
    {
      ...userModelEntry({ providerId: "xkiro", upstreamId: "operator-label-test", priority: 118 }),
      displayName: "My Xkiro Preview",
    },
  ...
  // Historical user state keeps its routing id, but the untouched curated
  // suffix is replaced at presentation time by the serving provider.
  assert.equal(
    registry.MODEL_BY_SLUG.get("opencode-free/x-preview-f-free").displayName,
    "x-preview-f-free (OpenCode Free)",
  );
  assert.equal(
    registry.MODEL_BY_SLUG.get("xkiro/anthropic/provider-label-test").displayName,
    "anthropic/provider-label-test (Xkiro API)",
  );
  assert.equal(
    registry.MODEL_BY_SLUG.get("commandcode-messages/anthropic/messages-provider-label-test").displayName,
    "anthropic/messages-provider-label-test (Command Code)",
  );
  assert.equal(
    registry.MODEL_BY_SLUG.get("xkiro/operator-label-test").displayName,
    "My Xkiro Preview",
  );
