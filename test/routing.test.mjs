...
test("routed parent task cache lets an encrypted child start without native session sharing", async () => {
  ...
  const gateway = await mockServer(async (request, response) => {
    ...
    if (gatewayRequests.length === 1) {
      const argumentsText = JSON.stringify({
        ...
        fork_turns: "all",
        message: "Inspect /tmp/capture.png harshly.",
        task_name: "critic",
      });
  ...
  try {
    await waitFor(`${routerBase(routerPort)}/models`, router);
    const parentThread = "019f8821-881a-7582-9e60-633bff68789f";
    const parentResponse = await fetch(`${routerBase(routerPort)}/responses`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${CALLER_KEY}`,
        "Content-Type": "application/json",
    ...
    const parentBody = await parentResponse.text();
    assert.equal(parentResponse.status, 200, parentBody);
    assert.match(parentBody, /"name":"spawn_agent"/);
    assert.match(parentBody, /"namespace":"collaboration"/);

    const childResponse = await fetch(`${routerBase(routerPort)}/responses`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${CALLER_KEY}`,
        "Content-Type": "application/json",
        "Thread-Id": "019fa7d4-e478-7640-be48-1b2698b4a975",
        "X-Codex-Parent-Thread-Id": parentThread,
        "X-OpenAI-Subagent": "critic",
      },
      body: JSON.stringify({
        model: "kimi-oauth/k3",
        stream: false,
        input: [
          {
            ...
            content: [
              ...
              { type: "encrypted_content", encrypted_content: "gAAAAA-native-child-token=" },
            ],
          },
        ],
      }),
    ...
    assert.equal(nativeRequests, 0, "native payload relay should be bypassed by the parent cache");
    assert.equal(gatewayRequests.length, 2);
    const childContent = gatewayRequests[1].body.input[0].content;
    assert.equal(childContent.some((part) => part.type === "encrypted_content"), false);
    assert.equal(childContent.at(-1).text, "Inspect /tmp/capture.png harshly.");
  } finally {
    await stopChild(router);
    await Promise.all([closeServer(native.server), closeServer(gateway.server)]);
  }
...
test("API forwarder blocks oversized Xkiro Opus 5 payloads before the provider call", async () => {
  const testRoot = mkdtempSync(path.join(os.tmpdir(), "api-forwarder-xkiro-limit-"));
  writeFileSync(
    ...
    `${JSON.stringify({
      ...
      models: [
        {
          slug: "xkiro/anthropic/claude-opus-5",
          gatewayModel: "xkiro-anthropic-claude-opus-5",
          compHash: "xkiro-anthropic-claude-opus-5-user-v1",
          upstreamModel: "anthropic/claude-opus-5",
          provider: "xkiro",
          listed: true,
          displayName: "Opus 5 (Xkiro)",
  ...
  const forwarder = run("api-forwarder.mjs", {
    CODEX_ROUTER_API_PORT: String(forwarderPort),
    CODEX_ROUTER_STATE_DIR: testRoot,
    XKIRO_API_BASE_URL: `http://127.0.0.1:${upstream.port}`,
    XKIRO_API_KEY: "TEST_XKIRO_API_KEY",
    CODEX_ROUTER_QUIET: "1",
  });

  async function send(content) {
    return fetch(`http://127.0.0.1:${forwarderPort}/v1/chat/completions`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${INTERNAL_KEY}`,
        "Content-Type": "application/json",
  ...
  try {
    ...
    const below = await send("x".repeat(1_140_000));
    assert.equal(below.status, 200);
    assert.equal(upstreamCalls, 1);

    const above = await send("x".repeat(1_160_000));
    assert.equal(above.status, 400);
    const error = await above.json();
    assert.equal(error.error?.type, "invalid_request_error");
    assert.equal(error.error?.code, "context_length_exceeded");
    assert.equal(error.error?.param, "input");
    assert.match(error.error?.message || "", /^context_length_exceeded:/);
    assert.equal(upstreamCalls, 1, "oversized request must not reach Xkiro");
  } finally {
    await stopChild(forwarder);
    await closeServer(upstream.server);
    rmSync(testRoot, { recursive: true, force: true });
...
test("reserve redirect restores the persisted routed selection without hijacking real GPT slugs", async () => {
  const gatewayRequests = [];
  const nativeRequests = [];
  const gateway = await mockServer(async (request, response) => {
    gatewayRequests.push(await bodyJson(request));
  ...
  const routerPort = await openPort();
  const testRoot = mkdtempSync(path.join(os.tmpdir(), "codex-router-reserve-redirect-"));
  const stateDir = path.join(testRoot, "state");
  ...
  async function send(model, input) {
    return fetch(`${routerBase(routerPort)}/responses`, {
      method: "POST",
      headers: {
        Authorization: "Bearer CODEX_CALLER_SECRET",
        "Content-Type": "application/json",
  ...
  try {
    ...
    const reserve = await send("gpt-reserve", "restore routed picker selection");
    assert.equal(reserve.status, 200);
    assert.equal(gatewayRequests.at(-1).model, "kimi-oauth-k3");
    assert.equal(nativeRequests.length, 0);

    const terra = await send("gpt-5.6-terra", "keep real GPT identity");
    assert.equal(terra.status, 200);
    assert.equal(nativeRequests.at(-1).model, "gpt-5.6-terra");
    assert.equal(gatewayRequests.length, 1, "real GPT traffic must never use the reserve redirect");
  } finally {
    await stopChild(router);
    await closeServer(gateway.server);
    await closeServer(native.server);
