...
test("release defaults contain no maintainer-specific project path", () => {
  const ipc = readScript("apps", "control-center", "electron", "ipc.mjs");
  const readme = readScript("README.md");
  for (const source of [ipc, readme]) {
    assert.doesNotMatch(source, /C:\\Users\\73428/i);
    assert.doesNotMatch(source, /F:\\\\程序\\\\office-leasing-ai/i);
  }
