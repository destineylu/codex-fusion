...
test("parent-only v2 opt-in fixes the parent catalog without creating a child assignment", () => {
  setMultiAgentMode("proven");
  setMultiAgentParentModel("xkiro/anthropic/claude-opus-5", true);
  const settings = readMultiAgentSettings();
  assert.deepEqual(settings.parentEnabled, ["xkiro/anthropic/claude-opus-5"]);

  const models = [
    { slug: "xkiro/anthropic/claude-opus-5" },
    { slug: "commandcode/deepseek-v4-flash", multiAgentVersion: "v2" },
  ];
  const childCapabilities = applyMultiAgentCapabilities(models, settings, { proofs: {} });
  assert.equal(childCapabilities[0].multiAgentVersion, undefined);
  assert.deepEqual(
    subagentEligibleModels(childCapabilities, settings).map((model) => model.slug),
    ["commandcode/deepseek-v4-flash"],
  );

  const parentCatalog = applyMultiAgentParentCapabilities(childCapabilities, settings);
  assert.equal(parentCatalog[0].multiAgentVersion, "v2");
  assert.equal(parentCatalog[0].multiAgentParentSelectedByOperator, true);
  assert.equal(parentCatalog[1].multiAgentVersion, "v2");

  // Ordinary child-selection writes must carry the independent parent choice.
  setMultiAgentModel("commandcode/deepseek-v4-flash", true);
  assert.deepEqual(readMultiAgentSettings().parentEnabled, ["xkiro/anthropic/claude-opus-5"]);
  setMultiAgentParentModel("xkiro/anthropic/claude-opus-5", false);
});
