...
test("cache-write usage survives both OpenAI and Anthropic Command Code bridges", () => {
  const events = [
    { type: "start" },
    { type: "text-start", id: "txt-0" },
    { type: "text-delta", id: "txt-0", text: "OK" },
  ...
  const openai = openAiChunks(run(events).sse).at(-1).usage;
  assert.deepEqual(openai.prompt_tokens_details, {
    cached_tokens: 60,
    cache_write_tokens: 15,
  });

  const anthropic = anthropicEvents(run(events, { protocol: "anthropic" }).sse)
    .find((entry) => entry.event === "message_delta").data.usage;
