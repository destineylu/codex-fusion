...
test("reserve bridge borrows only gpt-reserve while keeping truthful external identity", () => {
  const reserve = {
    ...template,
    slug: "gpt-reserve",
  ...
  const { models, aliases } = buildReserveBridgeCatalog(
    { models: [reserve, terra, template] },
    [grok],
    grok.slug,
  );
  assert.deepEqual(aliases, { "gpt-reserve": grok.slug });
  const bridged = models.find((model) => model.slug === "gpt-reserve");
  assert.equal(bridged.display_name, "Grok 4.5 (OAuth) (Reserve bridge)");
  assert.equal(bridged.visibility, "list");
  assert.match(bridged.description, /Grok through OAuth/);
  assert.match(bridged.description, /Luna Reserve transport slot/);
  assert.equal(models.find((model) => model.slug === grok.slug)?.visibility, "hide");
  assert.equal(models.find((model) => model.slug === "gpt-5.6-terra")?.display_name, "GPT-5.6-Terra");
  assert.equal(aliases["gpt-5.6-terra"], undefined);
});
