...
ensure_reference_upstream() {
  directory=$1
  if git -C "$directory" remote get-url upstream >/dev/null 2>&1; then
    return
  fi
  git -C "$directory" remote add upstream "$reference_upstream_url" ||
    die "unable to add the reference upstream remote"
}
...
if [ -z "$repo_dir" ]; then
  ...
  if [ -d "$install_dir/.git" ]; then
    ...
    [ "$current_branch" = "$update_branch" ] ||
      die "$install_dir must be on its $update_branch branch before updating"
    # Running the enhanced installer is an explicit request to follow the
    # enhanced distribution. Mutate remotes only after the dirty/branch refusal
    # gates have passed so a refused update stays read-only.
    if [ "$repository_url" = "$distribution_repository_url" ]; then
      case "$origin_url" in
        https://github.com/duolahypercho/codex-router|https://github.com/duolahypercho/codex-router.git|git@github.com:duolahypercho/codex-router.git)
          git -C "$install_dir" remote set-url origin "$distribution_repository_url" ||
            die "unable to repoint origin to the enhanced distribution"
          ;;
      esac
    fi
    ensure_reference_upstream "$install_dir"
    printf 'Updating %s...\n' "$install_dir"
    ...
    git clone --depth 1 --branch "$update_branch" "$repository_url" "$install_dir"
    ensure_reference_upstream "$install_dir"
  fi
