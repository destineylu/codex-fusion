...
- Use its impact analysis and risk-proportional verification before calling such
  ...
  edits.
- Before any Windows Codex Router upgrade, reinstall, Control Center rebuild,
  or large upstream adoption, read `docs/LOCAL-UPGRADE-PRESERVATION-2026-09-05.md`.
  In particular, preserve or consciously DROP/PORT the local Control Center
  Codex Native Agent Mode documented in
  `docs/LOCAL-PATCH-CONTROL-CENTER-CODEX-AGENT-MODE-2026-09-07.md`; it must stay
  independent from Router `setSubagentMode`. Do not rely on a maintainer-specific
  external recovery path; the checked-in preservation documents are the portable
  source of truth for this distribution.
...
5. For Kimi OAuth, reuse a valid `kimi login` session. If login is needed, run
   ...
   For Antigravity OAuth, accept the Google OAuth client secret either from
   `ANTIGRAVITY_CLIENT_SECRET` or from the router-managed owner-only
   `antigravity-client-secret` state file. Control Center may collect that
   secret through its password field and pipe it over stdin; it must never be
   placed in command arguments, logs, renderer state, or chat. Antigravity uses
   the router's own browser OAuth flow rather than a vendor CLI. Sign-in may provision a Google Cloud project
   for the account when none exists. The
   router-managed secret remains available to the background service for token
   refresh without requiring a shell environment or service reinstall.
...
4. If the provider is registered but the model is not checked in, run
   ...
   `node .\src\curate-models.mjs` with the same arguments. When the operator
   wants live evidence before adding an unregistered model, use
   `./bin/codex-router verify-model PROVIDER ID --live --yes`; it works for a
   provider with one curation route (for example Xkiro) as well as a family with
   several protocol variants. If discovery withholds an advertised model
   because a multi-protocol family has not settled its wire route, use this
   verifier instead of weakening the curation gate. It spends provider quota to
   prove the exact ID on each available route, streaming, and tool calling;
   `--apply` is allowed only after exactly one route passes, writes a local
   curated entry, publishes/restarts the shared routing plane, and then runs the
   full routed compatibility test. A failed final test rolls the local addition
   back. This evidence is machine-local curation permission, not checked-in
   registry certification or native-v2 subagent proof.
...
2. **Tray setup section.** Every provider must appear in the macOS tray with a
   ...
   - OAuth providers additionally get the OAuth section. CLI-owned OAuth uses
     an `OAUTH_CLIS` entry in `src/provider-onboarding.mjs` (executable, npm
     package, login arguments) so install/login buttons work, plus status,
     session-refresh, and reconnect-on-expiry wiring (follow `kimi-oauth-*` /
     `grok-oauth-*`). Router-owned browser OAuth such as `antigravity-oauth`
     is the exception: it has no vendor CLI, so Control Center must invoke the
     router's own `login` flow directly on every supported desktop platform.
...
7. **Provider Chat function names stop at 64 characters.** Real Codex tool
   inventories can exceed that after MCP/app namespace flattening; Command Code
   rejects such a request with HTTP 400 before the model runs. Bound only the
   Command Code Chat surface with `flattenNamespaceTools(..., { maxNameLength:
   64 })` and keep the request-local alias map authoritative for stored history,
   forced `tool_choice`, and the returned function call. Never truncate names:
   deterministic hashed aliases avoid collisions and the response must restore
   the exact native identity before Codex executes it. The routed compatibility
   probe deliberately uses an over-64-character Command Code tool name so this
   boundary cannot regress behind the short historical probe.
8. **Billing differs even though the models do not.** The Provider plan pays
