...
- **Native GPT account switching can now hand off an unfinished Codex task to another native ChatGPT account without creating a new thread.** Control Center can switch A → B and reopen the same `codex://threads/<id>`; Router treats B's first turn as a stateless handoff, drops only account-local bare `rs_*` stored references that B cannot resolve, preserves the rest of the conversation/tool history, and commits thread ownership to B only after the target account accepts the request. Auto Resume is explicitly secondary: if its binding or watcher recovery fails, the native thread handoff remains valid. Windows Desktop detection now uses `Win32_Process.ExecutablePath` so npm `codex.exe app-server` processes no longer falsely block switching when `Get-Process .Path` is unreadable.
