...
import {
  ...
  emptyCompletionPreludeMsForRoute,
...
} from "./provider-payload-limits.mjs";
...
import { fetchWithRetry, sleep as retrySleep } from "./upstream-retry.mjs";
import {
...
import { contextEconomyPolicy } from "./context-economy.mjs";
...
const EMPTY_COMPLETION_PRELUDE_MS =
  ...
  configuredEmptyCompletionPreludeMs >= 0
    ...
    : 30_000;
const configuredSlowFirstTokenPreludeMs = Number(
  process.env.CODEX_ROUTER_SLOW_FIRST_TOKEN_PRELUDE_MS ||
    process.env.CODEX_ROUTER_COMMANDCODE_MUSE_PRELUDE_MS ||
    90_000,
);
const SLOW_FIRST_TOKEN_PRELUDE_MS =
  Number.isFinite(configuredSlowFirstTokenPreludeMs) &&
  configuredSlowFirstTokenPreludeMs >= 0
    ? configuredSlowFirstTokenPreludeMs
    : 90_000;
...
function xkiroCompactionDuplicateRetryDelays() {
  const configured = String(
    process.env.CODEX_ROUTER_XKIRO_COMPACTION_DUPLICATE_RETRY_DELAYS_MS || "",
  )
    .split(",")
    .map((value) => Number(value.trim()))
    .filter((value) => Number.isFinite(value) && value >= 0)
    .slice(0, 4)
    .map((value) => Math.min(60_000, Math.floor(value)));
  return configured.length > 0 ? configured : [2_000, 8_000, 20_000, 40_000];
}

const XKIRO_COMPACTION_DUPLICATE_RETRY_DELAYS_MS =
  xkiroCompactionDuplicateRetryDelays();
...
const SPAWN_TASK_CACHE_MAX_BYTES = 4 * 1024 * 1024;
const SPAWN_TASK_CACHE_MAX_ENTRIES = 256;
const spawnTaskCache = new Map();
let spawnTaskCacheBytes = 0;
const spawnTaskCacheMetrics = {
  hits: 0,
  misses: 0,
  expirations: 0,
  evictions: 0,
...
function resourceLimitsPayload() {
  ...
  return {
    ...
    spawnTaskCache: {
      entries: spawnTaskCache.size,
      bytes: spawnTaskCacheBytes,
      maxEntries: SPAWN_TASK_CACHE_MAX_ENTRIES,
      maxBytes: SPAWN_TASK_CACHE_MAX_BYTES,
      ttlMs: AGENT_PAYLOAD_CACHE_TTL_MS,
      ...spawnTaskCacheMetrics,
    },
  };
}
...
function canonicalAgentTaskName(value) {
  if (typeof value !== "string") return undefined;
  const trimmed = value.trim();
  if (!trimmed) return undefined;
  const pieces = trimmed.split("/").filter(Boolean);
  return pieces.at(-1) || undefined;
}

function taskNameFromAgentEnvelope(visibleText) {
  if (typeof visibleText !== "string") return undefined;
  const match = visibleText.match(/(?:^|\n)Task name:\s*([^\r\n]+)\s*(?:\r?\n|$)/i);
  return canonicalAgentTaskName(match?.[1]);
}
...
function encryptedAgentPayload(item) {
  ...
  return {
    ...
    taskName: taskNameFromAgentEnvelope(visibleText),
  };
}
...
function spawnTaskCacheKey(parentThreadId, taskName) {
  if (typeof parentThreadId !== "string" || !parentThreadId) return undefined;
  const canonicalTask = canonicalAgentTaskName(taskName);
  if (!canonicalTask) return undefined;
  return createHash("sha256")
...
function evictSpawnTask(key, { expired = false, evicted = false } = {}) {
  const entry = spawnTaskCache.get(key);
  if (!entry) return;
  spawnTaskCache.delete(key);
  spawnTaskCacheBytes = Math.max(0, spawnTaskCacheBytes - (entry.bytes || 0));
  if (expired) spawnTaskCacheMetrics.expirations += 1;
  if (evicted) spawnTaskCacheMetrics.evictions += 1;
}

function purgeExpiredSpawnTasks(now = Date.now()) {
  for (const [key, entry] of spawnTaskCache) {
    if (entry.expiresAt <= now) evictSpawnTask(key, { expired: true });
  }
}

function rememberSpawnTask(parentThreadId, item) {
  if (
  ...
  const taskName = canonicalAgentTaskName(args.task_name);
  const message = typeof args.message === "string" ? args.message : undefined;
  const key = spawnTaskCacheKey(parentThreadId, taskName);
  const shape =
    message === undefined || message.length === 0
      ? "missing"
      : isNativeEncryptedToken(message)
        ? "native-token"
        : "plaintext";
  console.error(
    `[codex-router] subagent-handoff-cache parent=${key ? "scoped" : "missing"} ` +
      `task=${taskName ? "present" : "missing"} message=${shape}`,
  );
  if (!key || shape !== "plaintext") return;
  purgeExpiredSpawnTasks();
  const bytes = Buffer.byteLength(message, "utf8");
  if (bytes > SPAWN_TASK_CACHE_MAX_BYTES) return;
  const existing = spawnTaskCache.get(key);
  if (existing) evictSpawnTask(key);
  spawnTaskCache.set(key, {
    plaintext: message,
    bytes,
    expiresAt: Date.now() + AGENT_PAYLOAD_CACHE_TTL_MS,
  });
  spawnTaskCacheBytes += bytes;
  while (
    spawnTaskCache.size > SPAWN_TASK_CACHE_MAX_ENTRIES ||
    spawnTaskCacheBytes > SPAWN_TASK_CACHE_MAX_BYTES
  ) {
    const oldestKey = spawnTaskCache.keys().next().value;
    evictSpawnTask(oldestKey, { evicted: true });
  }
}

function cachedSpawnTask(parentThreadId, taskName) {
  purgeExpiredSpawnTasks();
  const key = spawnTaskCacheKey(parentThreadId, taskName);
  if (!key) {
    spawnTaskCacheMetrics.misses += 1;
    return undefined;
  }
  const entry = spawnTaskCache.get(key);
  if (!entry) {
  ...
  spawnTaskCache.delete(key);
  spawnTaskCache.set(key, entry);
  spawnTaskCacheMetrics.hits += 1;
  return entry.plaintext;
}
...
function parentThreadIdFromSubagentRequest(request) {
...
async function normalizeRoutedAgentInput(request, input, signal) {
  ...
  const output = [];
  const parentThreadId = parentThreadIdFromSubagentRequest(request);
  for (const item of normalized) {
    ...
    const spawnPlaintext = payload.native
      ? cachedSpawnTask(parentThreadId, payload.taskName)
      : undefined;
    if (payload.native) {
      console.error(
        `[codex-router] subagent-handoff-cache child=${spawnPlaintext === undefined ? "miss" : "hit"} ` +
          `task=${payload.taskName ? "present" : "missing"}`,
      );
    }
    const plaintext = payload.native
      ? spawnPlaintext ?? await relayEncryptedAgentPayload(request, item, payload.content, signal)
      : payload.content;
    output.push({
      ...item,
      content: [
...
function compactionAttempts(route, aged, { allowFailover = true } = {}) {
  ...
  if (!failoverAuthorized(settings)) return [route];
...
function isXkiroDuplicateCompactionFailure(route, status, bodyText) {
  if (Number(status) !== 409) return false;
  if (providerForModel(route)?.ownedBy !== "xkiro") return false;
  const text = String(bodyText || "").toLowerCase();
  return text.includes("duplicate request") && text.includes("already being processed");
}
...
async function summarizeWith(request, payload, route, aged, prepared, signal) {
  ...
  const provider = providerForModel(route);
  if (!provider?.nativeSessionAuth) delete body.client_metadata;
  ...
  if (provider?.id === "fireworks") delete body.web_search_options;
  const serialized = JSON.stringify(body);
  const payloadLimit = payloadLimitExceeded(serialized, route, provider);
  if (payloadLimit) {
    return {
      bridged,
  ...
  const compactionBase = provider?.nativeSessionAuth ? API_FORWARD_BASE : GATEWAY_BASE;
  const upstream = await fetch(`${compactionBase}/responses`, {
    method: "POST",
    headers: routedHeaders(),
    body: serialized,
    signal,
    ...
    for (;;) {
      ...
      if (sent.payloadLimit) {
        last ??= {
          ok: false,
          status: 400,
          payload: payloadLimitContextError(sent.payloadLimit),
        ...
        logFailover(
          attemptRoute,
          attempts[index + 1],
          "compaction/provider_payload_limit",
          400,
          "retrying",
        );
        continue routeAttempts;
      }
      let bytes;
      try {
      ...
      if (
        isXkiroDuplicateCompactionFailure(attemptRoute, sent.upstream.status, bodyText) &&
        duplicateRetryIndex < XKIRO_COMPACTION_DUPLICATE_RETRY_DELAYS_MS.length
      ) {
        const delayMs = XKIRO_COMPACTION_DUPLICATE_RETRY_DELAYS_MS[duplicateRetryIndex];
        duplicateRetryIndex += 1;
        if (!QUIET) {
          console.error(
            `[codex-router] xkiro compaction duplicate still in flight model=${attemptRoute.slug} ` +
              `retry=${duplicateRetryIndex}/${XKIRO_COMPACTION_DUPLICATE_RETRY_DELAYS_MS.length} delay_ms=${delayMs}`,
          );
        }
...
async function buildRoutedRequest({
  request,
  payload,
  route,
  agedInput,
  ...
  if (chatCompletionsProvider) {
    ...
    const flattened = chatProviderToolSurface(tools, provider?.id, {
      input,
      toolChoice: payload.tool_choice,
      deferUnreferencedAppTools: contextEconomy?.leanDeferredAppTools === true,
    });
    namespacesFlattened = flattened.flattened;
    flattenedNamespaces = flattened.namespaces;
    if (route.slug === "xkiro/anthropic/claude-opus-5") {
      console.error(
        `[codex-router] namespace-trace request model=${route.slug} flattened=${namespacesFlattened} ` +
          `namespaces=${[...flattenedNamespaces.keys()].join(",") || "none"} ` +
          `collaboration=${[...(flattenedNamespaces.get("collaboration") || [])].join(",") || "none"}`,
      );
    }
  ...
  if (namespacesFlattened) {
    ...
    if (provider?.id === "groq" || provider?.id === "commandcode") {
      // Groq needs forced references to match its flattened app surface, while
      // Command Code may additionally replace >64-character tool names with
      // deterministic aliases. In both cases the tool list, stored history,
      // and tool_choice must name the same provider-visible function.
  ...
  // Ordinary routed providers do not consume Codex client_metadata and
  // strict upstreams reject the unknown field. Native-session providers are
  // the exception: their local bridge uses x-codex-turn-metadata carried in
  // client_metadata to preserve the real Codex turn identity.
  if (!provider?.nativeSessionAuth) delete routed.client_metadata;
  ...
  if (provider?.keyless && !provider?.nativeSessionAuth) {
  ...
  return {
    ...
    target: `${provider?.nativeSessionAuth ? API_FORWARD_BASE : GATEWAY_BASE}/responses`,
    headers: routedHeaders(),
    namespacesFlattened,
    flattenedNamespaces,
...
async function prepareRoutedRequest({
  ...
  const tokenMaxxing = routeAgingEnabled && tokenMaxxingActive({
    ...
    autoCompact: contextEconomy?.autoCompact ?? route.autoCompact,
    ...(contextEconomy ? { pressureRatio: contextEconomy.pressureRatio } : {}),
  });
  const aged = ageToolResults(normalizedInput, {
    enabled: routeAgingEnabled,
    tokenMaxxing,
    ...(contextEconomy
      ? {
          minBytes: contextEconomy.agingMinBytes,
          frontier: contextEconomy.agingFrontier,
        }
      : {}),
  });
...
async function attemptModelFailover({
  ...
  const settings = readFailoverSettings();
  if (!failoverAuthorized(settings)) return undefined;
...
async function handleResponses(request, response, requestUrl) {
  ...
  try {
    ...
    // Luna Reserve can replace a routed picker selection with gpt-reserve in
    // the Desktop composer before the turn reaches us. Restore only that one
    // transport slug from the persisted Codex picker selection. Real native GPT
    // slugs never enter this branch, so choosing Sol/Terra/Luna still means the
    // genuine OpenAI model and keeps the user's visible model identity honest.
    if (!registeredRoute && requestedModel === RESERVE_NATIVE_SLUG) {
      const reserveTarget = MODEL_BY_SLUG.get(readReserveRedirectTarget());
      if (reserveTarget && readProviderSelection().includes(reserveTarget.provider)) {
        registeredRoute = reserveTarget;
      }
    }
    ...
    if (route) {
      ...
      const settings = readFailoverSettings();
      const cooled = !exactRouteProbe && failoverAuthorized(settings)
        ...
        transforms.push(
          new NamespaceToolCallTransform(
            ...
            // not pick up the routed-provider rewrites on the way through.
            {
              pendingInterrupts,
              injectOnly: !route,
              onFunctionCall: route
                ? (item) => rememberSpawnTask(threadIdFromHeaders(request.headers), item)
                : undefined,
            },
          ),
        );
      ...
      const guard =
        route && EMPTY_COMPLETION_RETRY
          ? new EmptyCompletionGuard(contentType, {
              ...
              maxPreludeMs: emptyCompletionPreludeMsForRoute(route, providerForModel(route), {
                defaultMs: EMPTY_COMPLETION_PRELUDE_MS,
                slowRouteMs: SLOW_FIRST_TOKEN_PRELUDE_MS,
              }),
            })
          : undefined;
      if (guard) {
