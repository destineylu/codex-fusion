...
import { antigravityClientSecretStatus } from "./antigravity-client-secret.mjs";
...
export function antigravityOAuthHealth() {
  const tokenPath = antigravityTokenPath();
  const clientSecret = antigravityClientSecretStatus();
  if (!existsSync(tokenPath)) {
    return {
      status: "missing",
      detail: clientSecret.configured
        ? "no Antigravity credential file"
        : "no Antigravity OAuth client secret or credential file",
      fix: clientSecret.configured
        ? "Run the Antigravity sign-in flow"
        : "Save the Antigravity OAuth client secret, then run the sign-in flow",
    };
  }
  if (!clientSecret.configured) {
    return {
      status: "missing-client-secret",
      detail: "Antigravity OAuth session exists, but its client secret is unavailable for token refresh",
      fix: "Save the Antigravity OAuth client secret before the access token expires",
    };
  }
  let value;
  try {
