...
async function providerPayload(provider, identity) {
  ...
  if (provider.nativeSessionAuth) {
    ...
    if (!native?.authorization) {
      throw new Error(
        `${provider.displayName} discovery requires the local Codex session sharing authorization.`,
      );
    }
    headers = native;
  } else {
    headers = provider.authMode === "anonymous"
      ? {}
  ...
  let clientVersion;
  if (provider.nativeSessionAuth) {
    clientVersion = codexVersion()?.match(/\b(\d+\.\d+\.\d+(?:[-+][A-Za-z0-9.-]+)?)\b/)?.[1];
    if (!clientVersion) {
      throw new Error(`${provider.displayName} discovery could not determine the current Codex client version.`);
    }
  }
  const payload = await fetchUntrustedModelCatalog(`${baseUrl}/models`, {
    headers,
    allowPrivate: Boolean(provider.keyless),
    ...(clientVersion ? { clientVersion, codexNativeCatalog: true } : {}),
  });
  if (provider.nativeSessionAuth) {
    return {
      data: payload.data.filter((record) =>
        typeof record?.id === "string" && record.id.startsWith("chatgpt-web/")
      ),
    };
  }
  return payload;
