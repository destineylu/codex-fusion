...
const SLOW_FIRST_TOKEN_PRECONTENT_MS = 90_000;
const SLOW_FIRST_TOKEN_ROUTES = new Set([
  "commandcode\0meta/muse-spark-1.3",
  "commandcode\0meta/muse-spark-1.3-contributor",
  "xkiro\0anthropic/claude-opus-5",
  "xkiro\0x-ai/grok-4.6",
]);

export function emptyCompletionPreludeMsForRoute(
  route,
  provider,
  {
    defaultMs = MAX_PRECONTENT_MS,
    slowRouteMs = SLOW_FIRST_TOKEN_PRECONTENT_MS,
  } = {},
) {
  const baseline = Number.isFinite(defaultMs) && defaultMs >= 0 ? defaultMs : MAX_PRECONTENT_MS;
  const slowLimit =
