...
export function recordUsageEvent({
  ...
  const event = {
    ...
    ...(safeTokenCount(cachedInputTokens) !== undefined
      ...
      : {}),
    ...(safeTokenCount(cacheWriteInputTokens) !== undefined
      ? { cacheWriteInputTokens: safeTokenCount(cacheWriteInputTokens) }
      : {}),
  ...
  if (canonicalProviderId(event.provider) === "commandcode") {
  ...
  try {
    const events = usageEventLines()
      ...
      .map((event) => {
        ...
        const cachedInputTokens = safeTokenCount(event.cachedInputTokens);
        const cacheWriteInputTokens = safeTokenCount(event.cacheWriteInputTokens);
