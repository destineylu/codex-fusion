...
function defaultSettings() {
  return { version: 1, enabled: false, chain: [], defaulted: true };
}
...
// Cross-model recovery is allowed only after the operator has named the exact
// fallback model(s). `enabled: true` with an empty chain is intentionally inert:
// it must never hand model choice to the router's ranking logic and turn a
// visible model selection into a blind-box execution path.
export function failoverAuthorized(settings = readFailoverSettings()) {
  return settings?.enabled === true && Array.isArray(settings.chain) && settings.chain.length > 0;
}
