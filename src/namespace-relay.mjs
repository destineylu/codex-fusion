...
// Codex 26.901 can publish the built-in v2 collaboration surface to routed
// models as already-flattened ordinary functions instead of a native namespace
// object. Those names are reserved by Codex itself, and its executor still
// expects the native { namespace: "collaboration", name } identity on the
// response path. Recognize only the built-ins we have an execution contract
// for; never split arbitrary `foo__bar` names, because those may be literal
// user/MCP function names.
const CODEX_COLLABORATION_TOOL_NAMES = new Set([
  "spawn_agent",
  "list_agents",
  "wait_agent",
  "interrupt_agent",
...
const CODEX_PLAINTEXT_COLLABORATION_MESSAGE_TOOLS = new Set([
  "spawn_agent",
  "send_message",
  "followup_task",
]);
const NATIVE_ENCRYPTED_COLLABORATION_ARGUMENT = /^gAAAAA[A-Za-z0-9_-]+={0,2}$/;
const ROUTED_ENCRYPTED_HISTORY_PLACEHOLDER =
  "Historical encrypted collaboration payload omitted for this routed provider. " +
  "For any new delegation, write the actual task as ordinary plaintext from the current conversation; " +
  "do not copy an opaque gAAAAA token from history.";

function codexCollaborationWireChild(name) {
  if (typeof name !== "string") return undefined;
  const prefix = `collaboration${NAMESPACE_DELIMITER}`;
  if (!name.startsWith(prefix)) return undefined;
  const child = name.slice(prefix.length);
  return CODEX_COLLABORATION_TOOL_NAMES.has(child) ? child : undefined;
}
...
// Codex 0.153+ can represent a namespaced tool as an ordinary function carrying
// an explicit `namespace` field instead of wrapping functions inside a
// `type: "namespace"` container. That identity is authoritative, so it is safe
// to flatten generically: unlike parsing `foo__bar`, no namespace is guessed
// from a user-controlled function name.
function explicitNamespacedFunctionIdentity(tool) {
  if (tool?.type !== "function") return undefined;
  const namespace =
    typeof tool.namespace === "string" && tool.namespace ? tool.namespace : undefined;
  const name = providerFunctionName(tool);
  if (!namespace || typeof name !== "string" || !name) return undefined;
...
function initialFunctionIdentities(tools) {
  ...
  for (const tool of tools) {
    ...
    const explicit = explicitNamespacedFunctionIdentity(tool);
    if (explicit) {
      identities.set(
        nativeToolKey(explicit.namespace, explicit.name),
        {
          wireName: `${explicit.namespace}${NAMESPACE_DELIMITER}${explicit.name}`,
          native: explicit,
        },
      );
      continue;
    }
    const collaborationChild = codexCollaborationWireChild(name);
    const native = collaborationChild
      ? { namespace: "collaboration", name: collaborationChild }
      : { name };
    identities.set(
      nativeToolKey(native.namespace, native.name),
      { wireName: name, native },
    );
  }
  return identities;
}
...
function providerVisibleToolNames(tools) {
  ...
  for (const tool of tools) {
    ...
    const explicit = explicitNamespacedFunctionIdentity(tool);
    if (explicit) {
      names.add(`${explicit.namespace}${NAMESPACE_DELIMITER}${explicit.name}`);
      continue;
    }
    const name = providerFunctionName(tool);
...
// Before any semantic output, keep an undecided SSE frame bounded. LiteLLM's
// Responses <- Chat bridge echoes the request's `instructions` and complete
// `tools` catalog inside response.created / response.in_progress. A Codex
// Desktop tool surface with collaboration, apps, computer use, and MCP tools
// routinely exceeds 256 KiB; treating that valid prelude as oversized disables
// namespace restoration for the rest of the stream and leaks names such as
// `collaboration__spawn_agent` to Codex. Two MiB stays tightly bounded while
// covering routed request envelopes (Xkiro Opus 5 is already capped below this
// at the serialized-request boundary). Once a namespace rewrite, suppression,
// or injection has committed the wire shape, a later terminal event may carry
// the complete response and therefore shares the non-streaming JSON bound.
...
function isNativeEncryptedCollaborationArgument(value) {
  return typeof value === "string" && NATIVE_ENCRYPTED_COLLABORATION_ARGUMENT.test(value);
}

function plaintextCollaborationMessageSchema(schema, nativeName) {
  if (
    !CODEX_PLAINTEXT_COLLABORATION_MESSAGE_TOOLS.has(nativeName) ||
    !plainObject(schema) ||
    !plainObject(schema.properties) ||
    !plainObject(schema.properties.message)
  ) {
    return schema;
  }
  ...
  const routedHint =
    "For routed third-party parents, put the literal task text here in ordinary plaintext; " +
    "never copy opaque gAAAAA encrypted values from prior tool-call history.";
  const hinted = description.includes("routed third-party parents")
    ? description
    : `${description}${description ? " " : ""}${routedHint}`;
  if (!encrypted && hinted === description) return schema;
  const { encrypted: _encrypted, ...messageWithoutEncrypted } = message;
  return {
    ...schema,
    properties: {
      ...schema.properties,
      message: { ...messageWithoutEncrypted, description: hinted },
    },
  };
}

function plaintextCollaborationToolDefinition(tool, namespace, nativeName) {
  if (
  ...
  let next = tool;
  for (const key of ["parameters", "inputSchema"]) {
    const schema = next[key];
    if (schema === undefined) continue;
    const rewritten = plaintextCollaborationMessageSchema(schema, nativeName);
    if (rewritten === schema) continue;
    if (next === tool) next = { ...tool };
    next[key] = rewritten;
  }
  if (plainObject(next.function?.parameters)) {
    const parameters = plaintextCollaborationMessageSchema(
      next.function.parameters,
      nativeName,
    );
    if (parameters !== next.function.parameters) {
      if (next === tool) next = { ...tool };
      next.function = { ...next.function, parameters };
    }
  }
  return next;
}
...
function flattenNamespaceChild(namespace, fn, providerName) {
  ...
  return plaintextCollaborationToolDefinition(
    {
      ...fn,
...
export function flattenNamespaceTools(
  ...
  for (const tool of tools) {
    ...
    // Newer Codex Desktop builds can send the exact native identity as
    // `{type:"function", name:"spawn_agent", namespace:"collaboration"}`.
    // Chat Completions providers cannot preserve that field as a dispatch
    // identity, so flatten it exactly as a namespace container and retain the
    // request-local reverse mapping. Without this branch the namespace is
    // silently discarded and the returned `collaboration__spawn_agent` reaches
    // Codex as an unsupported ordinary function.
    const explicit = explicitNamespacedFunctionIdentity(repaired);
    if (explicit) {
      if (!namespaces.has(explicit.namespace)) namespaces.set(explicit.namespace, new Set());
      namespaces.get(explicit.namespace).add(explicit.name);
      const providerName = providerNameForNative(
        namespaces,
        explicit.namespace,
        explicit.name,
      );
      const { namespace: _nativeNamespace, ...withoutNamespace } = repaired;
      flattened.push(
        flattenNamespaceChild(explicit.namespace, withoutNamespace, providerName),
      );
      if (explicit.namespace === "collaboration" && explicit.name === "spawn_agent") {
        const schema = repaired?.parameters ?? repaired?.inputSchema;
    ...
    // Codex 26.901 may also hand routed models its v2 collaboration built-ins
    // in provider-wire form already. Keep the provider-visible function
    // unchanged, but register its native identity so response restoration and
    // stored history use the same contract as a namespace object. Mark the
    // surface as semantically flattened even though the function spelling is
    // unchanged; otherwise a later native history item would not be flattened
    // on the next request and LiteLLM would replay a bare `spawn_agent` example.
    const collaborationChild = codexCollaborationWireChild(name);
    if (collaborationChild) {
      if (!namespaces.has("collaboration")) namespaces.set("collaboration", new Set());
      namespaces.get("collaboration").add(collaborationChild);
      providerNameForNative(namespaces, "collaboration", collaborationChild);
      if (collaborationChild === "spawn_agent") {
        const schema = repaired?.parameters ?? repaired?.inputSchema;
        schemaStringValues(schema?.properties?.model, spawnAgentModels);
      }
      changed = true;
      flattened.push(
        plaintextCollaborationToolDefinition(
          repaired,
          "collaboration",
          collaborationChild,
        ),
...
function discoveredProviderTools(toolSpecs, namespaces) {
  ...
  for (const tool of toolSpecs) {
    ...
    if (explicit) {
      ...
      const { namespace: _nativeNamespace, ...withoutNamespace } = tool;
      discovered.push({
        tool: flattenNamespaceChild(explicit.namespace, withoutNamespace, providerName),
        native: explicit,
        nativeName: explicit.name,
        identity: nativeToolKey(explicit.namespace, explicit.name),
      });
      continue;
    }
...
function historicalCollaborationMessageTool(item, namespaces) {
  if (item?.type !== "function_call" || typeof item.name !== "string") return undefined;
  if (
    item.namespace === "collaboration" &&
    namespaces.get("collaboration")?.has(item.name) &&
    CODEX_PLAINTEXT_COLLABORATION_MESSAGE_TOOLS.has(item.name)
  ) {
    return item.name;
  }
  if (item.namespace !== undefined) return undefined;
  const wireChild = codexCollaborationWireChild(item.name);
  if (wireChild && CODEX_PLAINTEXT_COLLABORATION_MESSAGE_TOOLS.has(wireChild)) {
    return wireChild;
  }
  if (
    CODEX_PLAINTEXT_COLLABORATION_MESSAGE_TOOLS.has(item.name) &&
    namespaces.get("collaboration")?.has(item.name) &&
    !PLAIN_TOOL_NAMES.get(namespaces)?.has(item.name)
  ) {
    return item.name;
  }
  return undefined;
...
function sanitizeHistoricalCollaborationMessage(item, nativeName) {
  if (
    !nativeName ||
  ...
  if (!plainObject(args)) return item;
  let changed = false;
  if (isNativeEncryptedCollaborationArgument(args.message)) {
    args = { ...args, message: ROUTED_ENCRYPTED_HISTORY_PLACEHOLDER };
    changed = true;
  }
  if (item.encrypted_function_args !== undefined) changed = true;
  if (!changed) return item;
  const { encrypted_function_args: _encryptedFunctionArgs, ...rest } = item;
  return {
    ...rest,
...
export function flattenNamespacedHistory(input, namespaces) {
  ...
  return input.map((item) => {
    ...
    const nativeMessageTool = historicalCollaborationMessageTool(item, namespaces);
    const routedItem = sanitizeHistoricalCollaborationMessage(item, nativeMessageTool);
    const { name } = routedItem;
    if (typeof name !== "string") return routedItem;
    // The client stores namespaced calls as { name, namespace }.
    const namespace = routedItem.namespace;
    if (typeof namespace === "string" && namespaces.get(namespace)?.has(name)) {
      const { namespace: _namespace, ...rest } = routedItem;
    ...
    if (SPECIAL_FUNCTION_REFERENCES.has(item)) return routedItem;
    ...
    if (plainProviderName && plainProviderName !== name) {
      return { ...routedItem, name: plainProviderName };
    }
    ...
    // upstream.
    if (providerNames.has(name)) return routedItem;
    if (flattenedNames.has(name)) {
      const providerName = providerNameForWire(namespaces, name);
      return providerName && providerName !== name
        ? { ...routedItem, name: providerName }
        : routedItem;
    }
...
function markDirectPlaintextCollaborationMessage(item) {
  if (
    item?.type !== "function_call" ||
    item.namespace !== "collaboration" ||
    !CODEX_PLAINTEXT_COLLABORATION_MESSAGE_TOOLS.has(item.name) ||
  ...
  if (!plainObject(args) || typeof args.message !== "string" || !args.message) return item;
  if (isNativeEncryptedCollaborationArgument(args.message)) {
    if (item.encrypted_function_args === undefined) return item;
    const { encrypted_function_args: _encryptedFunctionArgs, ...rest } = item;
    return rest;
  }
...
function rewriteNamespaceFunctionCallItem(
  ...
  if (typeof item.name === "string" && item.name.startsWith("collaboration__")) {
    console.error(
      `[codex-router] namespace-trace response wire=${item.name} resolved=${
        resolved ? `${resolved.namespace || "plain"}.${resolved.name}` : "none"
      }`,
    );
  }
  if (resolved) {
  ...
  rewritten = markDirectPlaintextCollaborationMessage(rewritten);
...
export class NamespaceToolCallTransform extends Transform {
  ...
  constructor(namespaces, contentType = "", sessionModel, options = {}) {
    ...
    this.#onFunctionCall = typeof options.onFunctionCall === "function"
    ...
    if (sessionModel === "xkiro/anthropic/claude-opus-5") {
      console.error(
        `[codex-router] namespace-trace transport content_type=${declared || "none"} ` +
          `event_stream=${this.#eventStream} json_declared=${declared.includes("json")}`,
      );
    }
    // Some OpenAI-compatible gateways mislabel a streamed Responses body as
    // application/json even though the bytes are SSE (`data:` / `event:`).
    // Sniff every non-explicit-SSE response. Real JSON settles as `other`
    // immediately from its `{` / `[` prefix, while a mislabeled SSE stream can
    // still reach the namespace restore path instead of being buffered and
    // relayed unchanged.
  ...
  #flushTransform() {
    ...
    if (!this.#eventStream) {
      ...
      try {
        ...
        if (this.#sessionModel === "xkiro/anthropic/claude-opus-5") {
          console.error(
            `[codex-router] namespace-trace transport non_sse_json_parse_failed bytes=${body.length}`,
          );
        }
        this.push(body);
        return;
      ...
      if (!this.#injectOnly) {
        ...
        this.#notifyFunctionCalls(payload);
      }
  ...
  #unsafeSseFrame(frame, reason) {
    console.error(`[codex-router] namespace-trace sse-disabled reason=${reason}`);
    if (this.#semanticMutationCommitted) {
  ...
  #rewriteSseFrame(frame) {
    ...
    if (!dataLine) {
      if (this.#sessionModel === "xkiro/anthropic/claude-opus-5") {
        console.error(
          `[codex-router] namespace-trace transport sse_frame_without_data bytes=${frame.length} ` +
    ...
    try {
      ...
      if (this.#sessionModel === "xkiro/anthropic/claude-opus-5") {
        const terminalOutput = Array.isArray(sourceEvent?.response?.output)
          ? sourceEvent.response.output
          : undefined;
        console.error(
          `[codex-router] namespace-trace event type=${sourceEvent?.type || "none"} ` +
            `terminal_output=${terminalOutput ? terminalOutput.length : "none"} ` +
            `terminal_types=${terminalOutput ? terminalOutput.map((item) => item?.type || "none").join(",") : "none"}`,
        );
      }
      if (
        sourceEvent?.type === "response.output_item.added" ||
        sourceEvent?.type === "response.output_item.done"
      ) {
        const item = sourceEvent.item;
        console.error(
          `[codex-router] namespace-trace raw event=${sourceEvent.type} item_type=${item?.type || "none"} item_name=${item?.name || "none"} item_namespace=${item?.namespace || "none"}`,
        );
      }
      ...
      if (!this.#injectOnly) {
        ...
        this.#notifyFunctionCalls(event);
      }
      if (sourceEvent?.type === "response.output_item.added") {
    ...
    const identity =
      typeof item.call_id === "string" && item.call_id
        ...
        : typeof item.id === "string" && item.id
          ? item.id
          : undefined;
    if (identity && this.#notifiedFunctionCalls.has(identity)) return;
    try {
      this.#onFunctionCall(item);
      if (identity) this.#notifiedFunctionCalls.add(identity);
    } catch {
      // Observers are advisory. A bookkeeping failure must never corrupt the
      // provider response or change Codex tool dispatch semantics.
    }
  }

  #notifyFunctionCalls(event) {
    if (!event || typeof event !== "object") return;
    if (event.type === "response.output_item.done") {
      this.#notifyFunctionCall(event.item);
    }
    for (const output of [event.output, event.response?.output]) {
      if (!Array.isArray(output)) continue;
      for (const item of output) this.#notifyFunctionCall(item);
    }
  }
