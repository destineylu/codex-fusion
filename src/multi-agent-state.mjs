...
function defaultSettings() {
  return { version: 2, mode: "proven", enabled: [], disabled: [], parentEnabled: [] };
}
  ...
  if (existsSync(MULTI_AGENT_STATE_PATH)) {
    try {
      ...
      if (
        ...
        return {
          ...parsed,
          parentEnabled: Array.isArray(parsed.parentEnabled) ? parsed.parentEnabled : [],
        };
      }
    } catch {
...
export function setMultiAgentModels(slugs, enabled) {
  ...
  const next = {
    ...
    parentEnabled: [...new Set(current.parentEnabled || [])].sort(),
...
// Parent-side compatibility is a different question from whether a model is
// safe to spawn as a child. Recent Codex builds no longer execute the legacy
// `multi_agent_v1__spawn_agent` surface, while some routed parents still receive
// it when their catalog entry remains v1. An explicit parent-only opt-in lets
// those models receive the current v2 collaboration toolset without adding them
// to the managed child-agent definitions or the operator's child selection.
export function setMultiAgentParentModel(slug, enabled) {
  const value = String(slug || "").trim();
  if (!value) throw new Error("A model slug is required.");
  const current = readMultiAgentSettings();
  const parents = new Set(current.parentEnabled || []);
  if (enabled) parents.add(value);
  else parents.delete(value);
  writeSettings({
    ...current,
    version: 2,
    parentEnabled: [...parents].sort(),
  });
  return subagentSettingsSnapshot();
}
...
export function replaceMultiAgentState({ mode, enabled = [], disabled = [], efforts, parentEnabled }) {
  if (!SUBAGENT_MODES.includes(mode)) {
    throw new Error(`Unknown subagent mode "${mode}". Choose: ${SUBAGENT_MODES.join(", ")}`);
  }
  const current = readMultiAgentSettings();
  const carried = efforts === undefined ? current.efforts : efforts;
  const carriedParents = parentEnabled === undefined ? current.parentEnabled : parentEnabled;
  const next = {
    version: 2,
    mode,
    enabled: [...new Set(enabled)].sort(),
    disabled: [...new Set(disabled)].sort(),
    parentEnabled: [...new Set(carriedParents || [])].sort(),
...
// Parent-only v2 opt-ins are applied only to the catalog handed to Codex. They
// deliberately do not feed `subagentEligibleModels()`, so opting a strong
// routed model in as a parent fixes the legacy v1 collaboration surface without
// silently making that same model a managed child agent. This is an operator
// compatibility choice, not repository certification of the provider route.
export function applyMultiAgentParentCapabilities(models, settings) {
  const parents = new Set((settings || readMultiAgentSettings()).parentEnabled || []);
  if (!Array.isArray(models) || parents.size === 0) return models;
  let changed = false;
  const next = models.map((model) => {
    const slug = String(model?.slug || "");
    if (!parents.has(slug) || model.multiAgentVersion === "v2") return model;
    changed = true;
    return {
      ...model,
      multiAgentVersion: "v2",
      multiAgentParentSelectedByOperator: true,
    };
  });
