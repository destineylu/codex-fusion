...
const UPDATE_BRANCH = String(process.env.CODEX_ROUTER_UPDATE_BRANCH || "main").trim() || "main";
...
export function checkForUpdate() {
  ...
  const available = git(["rev-parse", `origin/${UPDATE_BRANCH}`]);
...
export function updateCheckout({ force = false } = {}) {
  ...
  if (!branch) {
    git(["switch", UPDATE_BRANCH], { inherit: true });
    branch = UPDATE_BRANCH;
  }
  if (branch !== UPDATE_BRANCH) {
    throw new Error(`Updates require the managed checkout to be on its ${UPDATE_BRANCH} branch.`);
  }
