...
function packagedGuiEnvironment() {
  ...
  // Electron will interpret `--query-lifecycle` as Node argv and never enter
  // the GUI lifecycle handler.
  delete environment.ELECTRON_RUN_AS_NODE;
  return environment;
}

function invokeControlCenterLimitedHelper(executable, argument) {
  // The canonical tray task is deliberately Interactive/Limited. An elevated
  // installer cannot reliably signal that medium-integrity Electron instance
  // through requestSingleInstanceLock(): Windows lets the high-integrity helper
  // observe the existing lock, but Electron's second-instance message does not
  // reach the lower-integrity primary. Launch only the two fixed single-instance
  // requests through Task Scheduler under the current user's Limited token.
  if (!sameWindowsExecutable(executable, TRAY_BINARY)) {
    throw new Error("The Limited Control Center helper may target only this checkout's canonical package.");
  }
  if (argument !== "" && argument !== "--quit-for-update") {
    throw new Error("The Limited Control Center helper received an unsupported argument.");
  }
  const helperTaskName = `${TRAY_TASK_NAME} Limited Helper ${process.pid}-${Date.now()}`;
  const script = [
    "$ErrorActionPreference = 'Stop'",
    "$taskName = $env:CODEX_ROUTER_TRAY_HELPER_TASK",
    "$execute = [IO.Path]::GetFullPath($env:CODEX_ROUTER_TRAY_EXECUTE)",
    "$argument = [string]$env:CODEX_ROUTER_TRAY_HELPER_ARGUMENT",
    "$principalName = [Security.Principal.WindowsIdentity]::GetCurrent().Name",
    "$registeredAt = Get-Date",
    "$registered = $false",
    "try {",
    "  if ([string]::IsNullOrEmpty($argument)) { $action = New-ScheduledTaskAction -Execute $execute } else { $action = New-ScheduledTaskAction -Execute $execute -Argument $argument }",
    ...
    "    if ($info.LastRunTime -ge $registeredAt.AddSeconds(-1)) { $started = $true }",
    "    if ($started -and $task.State.ToString() -ne 'Running') { break }",
    `    Start-Sleep -Milliseconds ${TASK_STATE_POLL_MS}`,
    "  } while ([DateTime]::UtcNow -lt $deadline)",
    "  if (-not $started) { throw 'The Limited Control Center helper did not start.' }",
    "  if ($task.State.ToString() -eq 'Running') { throw 'The Limited Control Center helper did not finish.' }",
    "  if ([int64]$info.LastTaskResult -ne 0) { throw \"The Limited Control Center helper exited with task result $($info.LastTaskResult).\" }",
    "} finally {",
    "  if ($registered) { Unregister-ScheduledTask -TaskName $taskName -Confirm:$false -ErrorAction Stop }",
    "}",
  ].join("\n");
  try {
    execFileSync(
      ...
      ["-NoLogo", "-NoProfile", "-NonInteractive", "-Command", script],
      {
        ...
        env: {
          ...process.env,
          CODEX_ROUTER_TRAY_HELPER_TASK: helperTaskName,
          CODEX_ROUTER_TRAY_EXECUTE: executable,
          CODEX_ROUTER_TRAY_HELPER_ARGUMENT: argument,
        },
        stdio: ["ignore", "ignore", "pipe"],
        timeout: 30_000,
        windowsHide: true,
    ...
    const detail = String(error?.stderr || "").trim();
    throw new Error(
      `The Control Center Limited-token helper failed.${detail ? ` ${detail}` : ""}`,
      { cause: error },
    );
  }
}

function requestControlCenterQuitForUpdate(executable) {
  invokeControlCenterLimitedHelper(executable, "--quit-for-update");
}
...
function drainCanonicalExecutable(executable, { schedulerOwned = false } = {}) {
  ...
  if (!exactUserExecutableOwnsPid(executable, lifecycle.pid)) {
    // The lifecycle file can outlive its process for a few milliseconds. On
    // Windows that PID may be reused immediately by an unrelated process; in
    // that case processIsRunning(pid) keeps the otherwise stale lifecycle state
    // looking live. Treat it as stale only after independently proving that the
    // scheduled task is not running and no exact Control Center executable is
    // alive. Any ambiguity still fails closed and never authorizes a force-stop.
    const schedulerState = schedulerOwned ? taskState() : undefined;
    if (schedulerOwned && schedulerState === undefined) {
      throw new Error(
        "Task Scheduler did not answer while verifying a reused Control Center lifecycle PID; it was not force-stopped.",
      );
    }
    const exactProcesses = exactCanonicalProcessCount(executable);
    if (exactProcesses === 0 && schedulerState !== "running") return;
    throw new Error("The Control Center lifecycle PID does not belong to the registered current-user executable.");
  }
  requestControlCenterQuitForUpdate(executable);
...
function openControlCenterWindow() {
  if (skipServiceManagerCall({ hostManaged: HOST_MANAGED })) return unavailableLifecycle();
  const registered = requireCanonicalRegisteredAction();
  const initial = queryControlCenterLifecycle(registered.execute);
  if (
    !lifecycleWasVerified(initial)
    || !initial.running
    || !initial.ready
    || !exactUserExecutableOwnsPid(registered.execute, initial.pid)
  ) {
    throw new Error("The canonical Control Center is not ready for a window-open request.");
  }
  if (initial.visible) return initial;

  invokeControlCenterLimitedHelper(registered.execute, "");
  const deadline = Date.now() + TASK_START_TIMEOUT_MS;
  for (;;) {
    const lifecycle = queryControlCenterLifecycle(registered.execute);
    if (
      lifecycleWasVerified(lifecycle)
      && lifecycle.running
      && lifecycle.ready
      && lifecycle.pid === initial.pid
      && exactUserExecutableOwnsPid(registered.execute, lifecycle.pid)
      && lifecycle.visible
    ) return lifecycle;
    if (!lifecycle.running || (lifecycle.pid !== null && lifecycle.pid !== initial.pid)) {
      throw new Error("The Control Center primary changed while opening its window.");
    }
    if (Date.now() >= deadline) {
      throw new Error("The Control Center did not report a visible window after the Limited-token open request.");
    }
    sleep(TASK_STATE_POLL_MS);
  }
}
...
if (
  ...
  console.error(
    "Usage: tray-service-windows.mjs install|install-electron|uninstall|start|stop|restart|open|validate|lifecycle|status|render|render-task|render-electron",
  );
  process.exit(2);
}
...
if (command === "render" || command === "render-task") {
  ...
  const lifecycle = openControlCenterWindow();
  process.stdout.write(`${JSON.stringify({ opened: true, lifecycle })}\n`);
