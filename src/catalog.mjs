...
import {
  readReserveBridge,
  RESERVE_BRIDGE_NATIVE_SLUG,
} from "./reserve-bridge.mjs";
import {
...
export function routedModel(template, model, behaviorTemplate = template) {
  ...
  const next = {
    ...
    auto_compact_token_limit: contextEconomy?.autoCompact ?? model.autoCompact,
...
export function buildReserveBridgeCatalog(native, routedModelsList, targetSlug) {
  const target = routedModelsList.find((model) => String(model.slug) === String(targetSlug));
  const nativeReserve = native.models.find(
    (model) => String(model.slug) === RESERVE_BRIDGE_NATIVE_SLUG,
  );
  const ordinary = buildMergedCatalog(native, routedModelsList);
  if (!target || !nativeReserve) return { models: ordinary, aliases: {} };

  // Luna Reserve's renderer gate keys off the slug, but the picker label and
  // behavioral metadata remain ours. Show the real routed model identity in
  // the one Reserve slot and use the alias only after the turn reaches Router.
  // No ordinary GPT slug is touched by this bridge.
  const bridge = {
    ...routedModel(
      nativeReserve,
      target,
      behaviorTemplateFor(native.models, target, nativeReserve),
    ),
    slug: RESERVE_BRIDGE_NATIVE_SLUG,
    display_name: `${target.displayName} (Reserve bridge)`,
    description: `${target.description} Uses the Luna Reserve transport slot while OpenAI native quota is exhausted.`,
    priority: nativeReserve.priority,
    visibility: "list",
  };
  const models = ordinary.map((model) => {
    if (String(model.slug) === RESERVE_BRIDGE_NATIVE_SLUG) return bridge;
    if (String(model.slug) === String(target.slug)) return { ...model, visibility: "hide" };
    return model;
  });
  return {
    models: sortCatalogModels(models),
    aliases: { [RESERVE_BRIDGE_NATIVE_SLUG]: target.slug },
  };
}
...
function main() {
  ...
  // A routed model may be explicitly opted in as a v2 parent without becoming
  // a managed child. Apply that compatibility overlay only to what Codex reads
  // as its model catalog; `routedModels` below remains the child-eligibility
  // source of truth used to generate agent definitions.
  const parentCapableCatalogModels = applyMultiAgentParentCapabilities(
    catalogModels,
    multiAgentSettings,
  );
  const reserveBridge = !loginFree ? readReserveBridge() : undefined;
  const bridgeTarget = reserveBridge?.model
    ? parentCapableCatalogModels.find((model) => String(model.slug) === reserveBridge.model)
    : undefined;
  const { models: merged, aliases } = loginFree
    ? buildLoginFreeCatalog(native, parentCapableCatalogModels)
    : bridgeTarget && openaiAuthenticated && routedCatalog
      ? buildReserveBridgeCatalog(native, parentCapableCatalogModels, bridgeTarget.slug)
      : {
          models: buildMergedCatalog(native, routedCatalog ? parentCapableCatalogModels : [], {
