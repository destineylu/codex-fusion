...
export async function basicProbe(provider, credential, model, options = {}) {
  try {
    ...
    return {
      ok: response.ok && text.includes(BASIC_MARKER),
      status: response.status,
      detail: response.ok ? (text.includes(BASIC_MARKER) ? "marker verified" : "marker missing") : errorDetail(payload, response.status),
    };
  } catch (error) {
...
function streamEvidence(protocol, body) {
  let streamedText = "";
  let completed = false;
  ...
  return {
    marker: streamedText.includes(STREAM_MARKER) || body.includes(STREAM_MARKER),
    completed,
  };
}
...
export async function streamingProbe(provider, credential, model, { fetchImpl = undiciFetch } = {}) {
  try {
    const response = await fetchImpl(endpointFor(provider), {
      method: "POST",
      headers: headersFor(provider, credential),
      body: JSON.stringify(streamBody(provider, model)),
      signal: AbortSignal.timeout(REQUEST_TIMEOUT_MS),
    });
    const body = await response.text();
    const protocol = protocolFor(provider);
    const evidence = streamEvidence(protocol, body);
    return {
...
export async function reasoningProbe(provider, credential, model, options = {}) {
  ...
  try {
    ...
    return {
      ok: response.ok && text.includes(BASIC_MARKER),
      status: response.status,
      detail: response.ok ? (text.includes(BASIC_MARKER) ? "high effort verified" : "marker missing") : errorDetail(payload, response.status),
    };
  } catch (error) {
    return { ok: false, status: 0, detail: trimDetail(error?.message || error) };
  }
