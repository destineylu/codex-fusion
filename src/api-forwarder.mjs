...
function normalizeBody(buffer, contentType, route) {
  ...
  // Ordinary API providers do not consume Codex client_metadata and strict
  // upstreams reject it. Native-session bridges are the exception: they use
  // client_metadata.x-codex-turn-metadata to preserve the Codex turn identity.
  if (!provider?.nativeSessionAuth) delete payload.client_metadata;
...
function upstreamHeaders(requestHeaders, body, apiKey, provider, extraHeaders = {}, endpoint = provider) {
  ...
  if (provider.nativeSessionAuth) {
    // The provider consumes the official Codex session rather than a provider API key.
    // `upstreamSession()` supplies the owner-approved native headers below; never
    // substitute the keyless placeholder (`local`) into Authorization.
  } else if (endpoint.authMode === "anonymous") {
...
async function upstreamSession(provider, credential, payload, options = {}, endpoint = provider) {
  if (provider.nativeSessionAuth) {
    const headers = nativeSessionHeaders();
    if (!headers?.authorization) {
      throw new Error(
        `${provider.displayName} requires the local Codex session sharing authorization before it can relay native catalog/auth headers.`,
      );
    }
    return { apiKey: "", baseUrl: providerBaseUrl(endpoint), headers };
  }
  if (provider.authProfile !== "github-copilot") {
...
async function handleRequest(request, response) {
  ...
  // Xkiro's Opus 5 route has a measured serialized-body ceiling well below its
  // advertised token context. Both its Chat Completions and Anthropic Messages
  // surfaces fail around 1.19 MB, so token-count-only compaction cannot protect
  // every code/tool-heavy turn. Refuse locally before any provider call and
  // before any response byte is relayed; Codex then gets one deterministic 400
  // instead of an HTTP-200 SSE failure followed by its five reconnect attempts.
  const payloadLimit = payloadLimitExceeded(
    normalized.body,
    normalized.model,
    normalized.provider,
  );
  if (payloadLimit) {
    console.error(
      `[api-forwarder] provider=${normalized.provider.id} model=${normalized.model.upstreamModel} ` +
        `payload_bytes=${payloadLimit.bytes} limit_bytes=${payloadLimit.maxBytes} blocked=provider_payload_too_large`,
    );
    writeJson(response, 400, payloadLimitContextError(payloadLimit));
    return;
  }
