...
function openBrowser(url) {
  // Control Center can ask the trusted child to keep the callback/network work
  // here while Electron itself opens the system browser. This avoids relying
  // on PowerShell UI activation from a Limited-token child process on Windows.
  if (process.env.CODEX_ROUTER_BROWSER_OPEN === "0") return;
  let command;
  let args;
  let env;
...
export function signInAntigravity({
  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(callback.port, callback.host, () => {
      // Control Center waits for this marker before opening the URL it already
      // observed above, so the browser can never outrun the loopback listener.
      process.stdout.write("Antigravity OAuth callback ready.\n");
      try {
