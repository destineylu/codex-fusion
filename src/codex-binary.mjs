...
import {
  WINDOWS_SPAWNABLE_EXTENSIONS,
  commandOnPath,
  preferSpawnablePath,
  spawnableCommand,
} from "./spawnable-command.mjs";
...
function uniqueCodexBinaries(paths, platform = process.platform) {
  const seen = new Set();
  const binaries = [];
  for (const candidate of paths || []) {
    if (!candidate || !existsSync(candidate) || isShimFile(candidate)) continue;
    if (
      platform === "win32" &&
      !WINDOWS_SPAWNABLE_EXTENSIONS.includes(path.extname(String(candidate)).toLowerCase())
    ) {
      continue;
    }
...
// config.toml is shared by the Desktop-bundled Codex and any standalone/npm
// CLI under the same CODEX_HOME. Schema probes therefore have to satisfy every
// installed binary, not merely whichever executable the router prefers for its
// own native calls. An explicit CODEX_BIN remains authoritative: tests and
// operators use it specifically to select one binary and must not accidentally
// inherit another installation from PATH.
export function codexBinariesForSharedConfig({
  platform = process.platform,
  explicitBinary = process.env.CODEX_BIN,
  candidatePaths,
  pathBinary,
  resolvedBinary,
} = {}) {
  if (explicitBinary) return uniqueCodexBinaries([explicitBinary], platform);
  const direct = candidatePaths ?? candidates();
  const fromPath = pathBinary === undefined ? commandOnPath("codex") : pathBinary;
  const resolved = resolvedBinary === undefined ? resolveRealCodex()?.file : resolvedBinary;
  return uniqueCodexBinaries([...direct, fromPath, resolved], platform);
}
