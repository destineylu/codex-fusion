...
import { CONFIG_PATH, STATE_DIR } from "./paths.mjs";

export const NATIVE_REDIRECT_PATH =
  process.env.MODEL_ROUTER_NATIVE_REDIRECT_STATE ||
  path.join(STATE_DIR, "native-redirect.json");

export const RESERVE_NATIVE_SLUG = "gpt-reserve";

// Luna Reserve can overwrite the Desktop composer's selected routed model with
// gpt-reserve before the turn reaches the router. The root Codex `model` value
// is the user's persisted picker selection, so it is the only model identity we
// are willing to restore here. The router still validates that this value is a
// registered, enabled external route before using it.
export function readReserveRedirectTarget(configPath = CONFIG_PATH) {
  if (!existsSync(configPath)) return undefined;
  try {
    const config = readFileSync(configPath, "utf8");
    const firstTable = config.search(/^\s*\[/m);
    const root = firstTable === -1 ? config : config.slice(0, firstTable);
    const model = root.match(/^\s*model\s*=\s*["']([^"']+)["']/m)?.[1]?.trim();
    if (!model || model === RESERVE_NATIVE_SLUG) return undefined;
    return model;
  } catch {
    return undefined;
  }
