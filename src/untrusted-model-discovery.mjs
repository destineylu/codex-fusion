...
export async function fetchUntrustedModelCatalog(endpoint, {
  ...
  clientVersion,
  codexNativeCatalog = false,
} = {}) {
  if (typeof fetchImpl !== "function") throw new Error("Model discovery requires a fetch implementation.");
  if (
    clientVersion !== undefined &&
    (typeof clientVersion !== "string" || !/^\d+\.\d+\.\d+(?:[-+][A-Za-z0-9.-]+)?$/.test(clientVersion))
  ) {
    throw new Error("Model discovery clientVersion is invalid.");
  }
  if (typeof codexNativeCatalog !== "boolean") {
    throw new Error("Model discovery codexNativeCatalog flag is invalid.");
  }
  ...
  try {
    for (let redirect = 0; ; redirect += 1) {
      ...
      if (clientVersion) requestUrl.searchParams.set("client_version", clientVersion);
      const response = await requestFetch(requestUrl.toString(), {
        method: "GET",
      ...
      if (codexNativeCatalog) {
        if (!payload || typeof payload !== "object" || Array.isArray(payload) || !Array.isArray(payload.models)) {
          throw new Error("Provider returned an invalid Codex native model catalog.");
        }
        payload = {
          data: payload.models.map((record) => {
            if (!record || typeof record !== "object" || Array.isArray(record)) return record;
            const slug = record.slug;
            if (typeof slug !== "string" || !slug.trim()) return record;
