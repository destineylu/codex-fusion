...
// Explicit outage-only bridge for Luna Reserve. It never changes the visible
// identity of native GPT models; it only lets the hidden gpt-reserve transport
// slot represent one explicitly selected routed model while native quota is
// exhausted.
export const RESERVE_BRIDGE_PATH = path.join(STATE_DIR, "reserve-bridge.json");
...
export const ANTIGRAVITY_CLIENT_SECRET_PATH =
  process.env.ANTIGRAVITY_CLIENT_SECRET_PATH || path.join(STATE_DIR, "antigravity-client-secret");
export const CODEX_PROVIDER_MODE_PATH = path.join(STATE_DIR, "codex-provider-mode.json");
// Root-level context overrides are user-owned, but while the Router publishes a
// multi-provider catalog they would override every model's own context/compact
// metadata. Park them reversibly for the whole Router lifetime, not only for
// login-free mode, so routed models can compact at their provider-safe limits.
export const ROUTER_CONTEXT_OVERRIDE_PATH = path.join(
  STATE_DIR,
  "router-context-overrides.json",
);
