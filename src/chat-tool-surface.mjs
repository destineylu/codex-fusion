...
export const COMMANDCODE_MAX_TOOL_NAME_LENGTH = 64;
...
export function chatProviderToolSurface(
  tools,
  providerId,
  { input, toolChoice, deferUnreferencedAppTools = false } = {},
) {
  if (providerId !== "groq" && deferUnreferencedAppTools) {
    // Context Economy keeps the live client surface intact but does not expand
    // every deferLoading Codex app definition into every paid third-party
    // turn. A definition already present in the live request is preserved; a
    // stored call or forced choice is evidence that an omitted definition is
    // required and adds only that definition back.
    ...
    return flattenNamespaceTools(
      ...
      providerId === "commandcode"
        ? { maxNameLength: COMMANDCODE_MAX_TOOL_NAME_LENGTH }
        : undefined,
    );
  }
  ...
  if (providerId !== "groq") {
    // Command Code's Chat surface enforces a 64-character function-name cap.
    // Codex can expose longer flattened MCP/app names, so use the namespace
    // relay's deterministic request-local aliases rather than truncating them.
    // The same alias map rewrites history/tool_choice and restores returned
    // calls to the exact native identity before Codex sees them.
    return flattenNamespaceTools(
      merged.tools,
      providerId === "commandcode"
        ? { maxNameLength: COMMANDCODE_MAX_TOOL_NAME_LENGTH }
