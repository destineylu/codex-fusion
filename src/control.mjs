...
async function emitProbe() {
  ...
  const { contextEconomySnapshot } = await import("./context-economy-state.mjs");
  const { contextEconomyPolicies } = await import("./context-economy.mjs");
        ...
        ? {
            ...
            modelSettings: {
              ...
              contextEconomy: {
                ...
                policies: contextEconomyPolicies(),
              },
...
async function handleSubagents(action, value, flag, rest = []) {
  ...
  if (action === "select-all") {
    ...
    if (!["on", "off"].includes(flag)) {
      throw new Error("Usage: control subagents parent <model-slug> <on|off>");
    }
    if (!(await knownModelSlug(value))) {
      throw new Error(`Unknown model slug: ${value}`);
    }
    // A checked-in v1 verdict is an explicit repository decision. Parent-only
    // compatibility is for unknown/local routed models whose current Codex
    // surface fell back to legacy multi_agent_v1; it must not override a
    // reviewed denial.
    if (flag === "on" && (await knownModelSubagentVersion(value)) === "v1") {
      throw new Error(
        `${value} is repository-certified v1 and cannot be opted into the v2 parent surface.`,
      );
    }
    const { setMultiAgentParentModel } = await import("./multi-agent-state.mjs");
    setMultiAgentParentModel(value, flag === "on");
    ...
    throw new Error(
      "Usage: control subagents status|select-all|unselect-all|mode <all|selected|proven>|" +
        "set <model-slug> <on|off>|parent <model-slug> <on|off>|" +
        "effort <model-slug> <level|default>|provider <provider-id> <on|off>|" +
        "verify [model-slug ...]|certify <model-slug>|" +
...
const CONTEXT_ECONOMY_USAGE = "Usage: control context-economy status|on|off";

async function handleContextEconomy(action) {
  const {
    contextEconomySnapshot,
    readContextEconomySettings,
    setContextEconomyEnabled,
  } = await import("./context-economy-state.mjs");
  const { contextEconomyPolicies } = await import("./context-economy.mjs");
  ...
  if (desired !== "on" && desired !== "off") throw new Error(CONTEXT_ECONOMY_USAGE);

  const previous = readContextEconomySettings();
  setContextEconomyEnabled(desired === "on");
  try {
    refreshModelSettingsCatalog();
  } catch (error) {
    // The setting and the published catalog are one operator action. Roll the
    // setting back if publication fails so the running router and picker do
    // not disagree about the intended working-context budget.
    setContextEconomyEnabled(previous.enabled === true);
    throw error;
  }
  process.stdout.write(`${JSON.stringify(snapshot())}\n`);
}
...
// Luna Reserve can hard-disable the Desktop composer for a canonical routed
// slug before a turn reaches Router. This outage-only bridge publishes exactly
// one truthful external model in the gpt-reserve transport slot. The picker
// label remains the real routed model name; only the hidden transport slug is
// borrowed, and ordinary GPT slugs remain native.
async function handleReserveBridge(action, value) {
  const {
    clearReserveBridge,
    reserveBridgeSnapshot,
    setReserveBridge,
  } = await import("./reserve-bridge.mjs");
  if (!action || action === "status") {
    process.stdout.write(`${JSON.stringify(reserveBridgeSnapshot())}\n`);
    return;
  }
  if (!["on", "off", "set"].includes(action)) {
    throw new Error("Usage: control reserve-bridge status|on [routed-model-slug]|set <routed-model-slug>|off");
  }
  const config = codexConfigSnapshot();
  if (action !== "off" && config?.login_free) {
    throw new Error("Reserve bridge is only for signed-in Hybrid mode; turn off login-free mode first.");
  }
  let target;
  if (action !== "off") {
    target = String(value || config?.model || "").trim();
    const { selectedConfiguredListedModels } = await import("./provider-selection.mjs");
    if (!selectedConfiguredListedModels().some((model) => String(model.slug) === target)) {
      throw new Error(
        `${target || "The current model"} is not an enabled, authenticated routed model. Select a third-party model first or pass its slug explicitly.`,
      );
    }
  }

  await withModelOverlayLock(async () => {
    const before = reserveBridgeSnapshot();
    if (action === "off") clearReserveBridge();
    else setReserveBridge(target);
    try {
      refreshModelSettingsCatalog();
    } catch (error) {
      if (before.enabled && before.model) setReserveBridge(before.model);
      else clearReserveBridge();
      try {
        refreshModelSettingsCatalog();
      } catch (rollbackError) {
        throw new AggregateError(
          [error, rollbackError],
          "Reserve bridge publication failed and its catalog rollback also failed.",
        );
      }
      throw error;
    }
...
} else if (args[0] === "context-economy") {
  await handleContextEconomy(args[1]);
...
} else if (args[0] === "reserve-bridge") {
  await handleReserveBridge(args[1], args[2]);
