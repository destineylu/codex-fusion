...
import { providerAutoCompactLimit } from "./provider-payload-limits.mjs";
...
function loadRegistry() {
  ...
  for (const provider of parsed.providers) {
    ...
    if (provider.kind === "openai-compatible") {
      ...
      if (
        provider.nativeSessionAuth !== undefined &&
        typeof provider.nativeSessionAuth !== "boolean"
      ) {
        fail(`provider ${provider.id} has an invalid nativeSessionAuth flag`);
      }
      ...
      if (provider.nativeSessionAuth) {
        if (!provider.keyless || !loopbackBaseUrl(provider.baseUrl)) {
          fail(`native-session provider ${provider.id} must be keyless and loopback-only`);
        }
        if (!["openai", "openai-responses"].includes(provider.protocol)) {
          fail(`native-session provider ${provider.id} requires an OpenAI-compatible protocol`);
        }
      }
...
function normalizedModel(model, provider, { curated = false, providers } = {}) {
  const officialDisplayName = curated
    ...
    : undefined;
  const untouchedCuratedName = `${model.upstreamModel} (curated)`;
  const canonicalProvider = curated && provider?.variantOf && providers instanceof Map
    ? providers.get(provider.variantOf) || provider
    : provider;
  const providerDisplayName = canonicalProvider?.displayName || canonicalProvider?.id || model.provider;
  const curatedDisplayName = curated && model.displayName === untouchedCuratedName
    ? `${model.upstreamModel} (${providerDisplayName})`
    : undefined;
  const resolvedDisplayName = officialDisplayName || curatedDisplayName;
  const presented = resolvedDisplayName && model.displayName !== resolvedDisplayName
    ? { ...model, displayName: resolvedDisplayName }
    : model;
  // A provider can advertise a very large token window while enforcing a much
  // smaller serialized-request ceiling. Keep the advertised contextWindow for
  // capability metadata, but lower autoCompact when a live-measured exact route
  // has such a ceiling so Codex summarizes before ordinary code/tool history
  // grows into it. The hard byte guard in api-forwarder.mjs is the final backstop.
  const autoCompactLimit = providerAutoCompactLimit(presented, provider);
  const bounded =
    Number.isInteger(autoCompactLimit) &&
    autoCompactLimit > 0 &&
    Number.isInteger(presented.autoCompact) &&
    presented.autoCompact > autoCompactLimit
      ? { ...presented, autoCompact: autoCompactLimit }
      : presented;
  if (!provider?.perModelEndpoint) return Object.freeze(bounded);
