...
import { commandCodePlanFromCreditsPayload } from "./commandcode-billing.mjs";
...
export function commandCodeCreditsMetrics(payload) {
  ...
  const balance = Number.isFinite(monthly)
    ...
    : [];
  const plan = commandCodePlanFromCreditsPayload(payload);
  const monthlyUsage = plan && Number.isFinite(monthly)
    ? quotaMetric(
        "Monthly plan usage",
        {
          limit: plan.monthlyCredits,
          used: Math.max(0, plan.monthlyCredits - monthly),
        },
        "credits",
      )
    : undefined;
...
async function commandCodeAccount(fetchImpl) {
  ...
  try {
    ...
    const plan = commandCodePlanFromCreditsPayload(payload);
    if (plan) account.plan = plan.name;
    return account;
  } catch {
