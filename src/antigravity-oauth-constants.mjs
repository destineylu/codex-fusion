...
import { readAntigravityClientSecret } from "./antigravity-client-secret.mjs";
...
export function requireAntigravityClientSecret() {
  const value = readAntigravityClientSecret();
  if (!value) {
    ...
    throw new Error(
      "Antigravity OAuth is not configured on this build: ANTIGRAVITY_CLIENT_SECRET is not set. " +
        "Set it in the environment or save it through Control Center before signing in. " +
        "A router-managed copy is owner-only and is available to the background service for token refresh.",
    );
  }
  return value;
}
