...
export async function providerUsageSnapshot(options = {}) {
  ...
  const commandCodeAccount = accounts.commandcode;
  const commandCodeMetric = (label) =>
    commandCodeAccount?.metrics?.find((metric) => metric?.kind === "quota" && metric.label === label);
  const windowStartFromReset = (metric, durationMs) => {
    const resetSeconds = Number(metric?.resetAt ?? metric?.resetsAt);
    if (!Number.isFinite(resetSeconds) || resetSeconds <= 0) return undefined;
    const resetMs = resetSeconds > 1e12 ? resetSeconds : resetSeconds * 1_000;
    return resetMs > now ? resetMs - durationMs : undefined;
  };
  const commandCodeSpend = aggregateCommandCodeSpend(retainedEvents, {
    now,
