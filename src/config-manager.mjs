...
// model may need a much earlier compact point than another. Router mode parks
// these exact user assignments for its full lifetime and restores them verbatim
// only when the Router is disabled. The older login-free state still carries
// the same snapshot for migration compatibility.
...
function readRouterContextOverrideState() {
  if (!existsSync(ROUTER_CONTEXT_OVERRIDE_PATH)) return undefined;
  try {
    const parsed = JSON.parse(readFileSync(ROUTER_CONTEXT_OVERRIDE_PATH, "utf8"));
    if (
      parsed?.version !== 1 ||
      !Array.isArray(parsed.overrides) ||
      !validLoginFreeContextOverrideSnapshot(parsed.overrides)
    ) {
      throw new Error("invalid state");
    }
    return parsed;
  } catch {
    throw new Error(`Invalid Router context-override state at ${ROUTER_CONTEXT_OVERRIDE_PATH}.`);
  }
}

function writeRouterContextOverrideState(value) {
  mkdirSync(path.dirname(ROUTER_CONTEXT_OVERRIDE_PATH), { recursive: true, mode: 0o700 });
  const temporary = `${ROUTER_CONTEXT_OVERRIDE_PATH}.tmp.${process.pid}`;
  ...
  try {
    ...
    renameSync(temporary, ROUTER_CONTEXT_OVERRIDE_PATH);
    protectPrivateFile(ROUTER_CONTEXT_OVERRIDE_PATH);
  } catch (error) {
...
function clearRouterContextOverrideState() {
  if (existsSync(ROUTER_CONTEXT_OVERRIDE_PATH)) unlinkSync(ROUTER_CONTEXT_OVERRIDE_PATH);
}

function sameContextOverrideLines(left, right) {
  return (
...
function parkRouterContextOverrides(contents, fallbackOverrides) {
  const current = loginFreeContextOverridesFromRoot(splitRoot(contents).rootLines);
  const existing = readRouterContextOverrideState();
  if (existing) {
    if (current.length && !sameContextOverrideLines(current, existing.overrides)) {
      throw new Error(
        "Router context overrides changed while the Router owns their parked state; refusing to discard user changes.",
      );
    }
    return {
      contents: current.length ? withoutLoginFreeContextOverrides(contents) : contents,
      state: existing,
      pending: false,
    };
  }
  const inherited = Array.isArray(fallbackOverrides) ? fallbackOverrides : [];
  if (!validLoginFreeContextOverrideSnapshot(inherited)) {
    throw new Error("Invalid inherited Router context-override snapshot.");
  }
  const overrides = current.length ? current : inherited;
  return {
    contents: current.length ? withoutLoginFreeContextOverrides(contents) : contents,
    state: { version: 1, overrides },
    pending: true,
  };
}

function restoreRouterContextOverrides(contents, state) {
  if (!state) return contents;
  const current = loginFreeContextOverridesFromRoot(splitRoot(contents).rootLines);
  if (current.length) {
    if (sameContextOverrideLines(current, state.overrides)) return contents;
    throw new Error(
      "Router context overrides changed while parked; refusing to overwrite user-owned context settings.",
    );
  }
  return restoreLoginFreeContextOverrides(contents, {
    previousContextOverrides: state.overrides,
  });
}
...
function snapshot(contents) {
  ...
  const routerContextState = readRouterContextOverrideState();
  const routerContextOverrides = loginFreeContextOverridesFromRoot(rootLines);
  return {
    mode:
    ...
    router_context_overrides_parked: Boolean(
      routerContextState && routerContextOverrides.length === 0,
    ),
    router_context_override_count: routerContextState?.overrides.length || 0,
...
let pendingRouterContextOverrideState;
let shouldClearRouterContextOverrideState = false;
if (command === "enable") {
  const signedState = readSignedProviderModeState();
...
// The published catalog is multi-provider, so root context overrides must not
// shadow each model's own context/compact metadata. Park them for every Router
// mode (ordinary, signed, or login-free) and restore them only when the Router
// itself is disabled. Older login-free snapshots seed the new Router-wide
// state during migration so their original values are not lost.
if (["enable", "login-free-enable", "login-free-disable", "signed-enable", "signed-disable"].includes(command)) {
  const providerStateForContext = readProviderModeState();
  const parked = parkRouterContextOverrides(
    next,
    providerStateForContext?.previousContextOverrides,
  );
  next = parked.contents;
  if (parked.pending) pendingRouterContextOverrideState = parked.state;
} else if (command === "disable") {
  const routerContextState = readRouterContextOverrideState();
  if (routerContextState) {
    next = restoreRouterContextOverrides(next, routerContextState);
    shouldClearRouterContextOverrideState = true;
  }
}
...
const previousRouterDefaultState = pendingRouterDefaultState
  ? readCodexRouterDefault()
  : undefined;
const previousRouterContextOverrideState = pendingRouterContextOverrideState
  ? readRouterContextOverrideState()
  : undefined;
...
if (pendingRouterContextOverrideState) {
  writeRouterContextOverrideState(pendingRouterContextOverrideState);
}
try {
  atomicWrite(next);
  ...
  if (pendingRouterContextOverrideState) {
    if (previousRouterContextOverrideState) {
      writeRouterContextOverrideState(previousRouterContextOverrideState);
    } else {
      clearRouterContextOverrideState();
    }
  }
...
if (shouldClearRouterContextOverrideState) clearRouterContextOverrideState();
