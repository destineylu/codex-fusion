...
export function StatusPage({
  ...
  const [autoResume, setAutoResume] = useState<CodexAutoResumeSnapshot>();
  ...
  useEffect(() => {
    ...
    if (!api || typeof api.getCodexAutoResume !== "function") {
      setAutoResume(undefined);
      return () => { active = false; };
    }
    void api.getCodexAutoResume().then((snapshot) => {
      if (active) setAutoResume(snapshot);
    }).catch(() => {
      if (active) setAutoResume(undefined);
    });
    return () => { active = false; };
  }, [api, refreshing]);
  ...
  return (
    <div className="usage-status-page status-page">
      ...
      <section className="panel-section st-auto-resume-panel" aria-label="Codex Auto Resume status">
        <SectionHeading
          title="Codex Auto Resume"
          description="Original-thread continuation after native Codex quota recovery. It is isolated from Router fallback and model switching."
          action={autoResume ? (
            <Badge tone={autoResume.running ? "success" : autoResume.installed ? "neutral" : "warning"}>
              {autoResume.running ? "Watching" : autoResume.installed ? "Stopped" : "Not installed"}
            </Badge>
          ) : undefined}
        />
        {!autoResume ? (
          <PanelSkeleton label="Loading Codex Auto Resume status" count={3} />
        ) : autoResume.installed ? (
          <div className="st-reset-list">
            <article>
              ...
              <time>
                <strong>{autoResume.autostart ? "Enabled" : "Disabled"}</strong>
                <small>{autoResume.running ? "Watcher is running" : "Watcher is not running"}</small>
              </time>
            </article>
            <article>
              <span>
                <strong>Tracked threads</strong>
                <small>{autoResume.lastStatus || "No resume event recorded yet"}</small>
              </span>
              <time>
                <strong>{autoResume.activeThreads}/{autoResume.trackedThreads}</strong>
                <small>{autoResume.lastCheckedAt ? formatDateTime(autoResume.lastCheckedAt * 1000) : "No check timestamp yet"}</small>
              </time>
            </article>
            <article>
              <span>
                <strong>Weekly reset credit</strong>
                <small>Automatic reset-credit redemption</small>
              </span>
              <time>
                <strong>{autoResume.autoRedeemWeeklyReset === true ? "Auto" : "Off"}</strong>
                <small>{autoResume.autoRedeemWeeklyReset === true ? "Explicitly enabled" : "Safe default"}</small>
              </time>
            </article>
          </div>
        ) : (
          <EmptyState
            icon={<Timer size={20} />}
            title="Auto Resume is not installed"
            body="Install the audited sidecar from Settings to monitor native Codex quota recovery."
          />
        )}
      </section>
