    ...
    <div ref={pageRef} tabIndex={-1} aria-label="Usage overview" className="usage-status-page usage-page">
      ...
      {!source ? (
        ...
        <>
          ...
          {providerUsage?.commandCodeSpend ? (
  ...
  return (
    <section className="panel-section us-commandcode-panel" aria-label="Command Code model spend">
      ...
      <dl className="us-spend-summary">
        <SpendFact
          label="Official plan used"
          value={monthly?.used == null ? "—" : formatCredits(monthly.used)}
          detail={monthly?.limit == null ? "Monthly account meter unavailable" : `of ${formatCredits(monthly.limit)} plan credits`}
        />
        <SpendFact
        ...
        <SpendFact
          ...
          detail="Model allowance multiplier applied; not a separate per-model wallet"
        />
        <SpendFact
          label="Unattributed"
          value={unattributed == null ? "—" : formatCredits(unattributed)}
          detail={unattributed == null
            ? "No directly comparable official window for this range"
            : "Official used minus locally attributable credits; includes pre-tracking, incomplete usage, or traffic outside this Router"}
        />
      </dl>
