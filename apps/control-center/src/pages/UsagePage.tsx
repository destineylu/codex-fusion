...
import type {
  AccountUsage,
  CommandCodeSpendSnapshot,
  CommandCodeSpendWindow,
  ProviderUsage,
...
export function UsagePage({
  ...
  const activeAllowanceProviderId = activeAllowanceGroup?.source.id.startsWith("provider:")
    ? activeAllowanceGroup.source.id.slice("provider:".length)
    : undefined;
  const showCommandCodeSpend = activeAllowanceProviderId === "commandcode";
  const activeXkiro = activeAllowanceProviderId === "xkiro" || activeAllowanceProviderId === "xkiro2"
    ? xkiroEntries.find((entry) => entry.id === activeAllowanceProviderId)
    : undefined;
    ...
    <div ref={pageRef} tabIndex={-1} aria-label="Usage overview" className="usage-status-page usage-page">
      ...
      {!source ? (
        ...
        <>
          ...
          {showCommandCodeSpend && providerUsage?.commandCodeSpend ? (
            <CommandCodeSpendPanel
              spend={providerUsage.commandCodeSpend}
...
function CommandCodeSpendPanel({
  spend,
  provider,
  range,
  onRangeChange,
}: {
  spend: CommandCodeSpendSnapshot;
  ...
  const monthly = metric("Monthly plan usage");
  const balance = metric("Plan credits");
  const fiveHour = metric("5-hour limit");
  const weekly = metric("Weekly limit");
  ...
  const unattributed = officialWindowUsed == null
    ? null
    : Math.max(0, officialWindowUsed - window.goatCredits);
  const tabs = [
    ["fiveHour", "5 hours"],
    ["weekly", "7 days"],
    ["thirtyDay", "30 days"],
  ...
  return (
    <section className="panel-section us-commandcode-panel" aria-label="Command Code model spend">
      <SectionHeading
        title="Command Code model spend"
        description={`${spend.plan || provider?.account?.plan || "Command Code"} account totals are provider-reported; model attribution is calculated locally from the Router ledger.`}
        action={(
          <div className="us-spend-range" role="group" aria-label="Command Code spend range">
            {tabs.map(([key, label]) => (
              <button
                key={key}
                type="button"
      ...
      <dl className="us-spend-summary is-windowed">
        {range === "fiveHour" ? (
          <SpendFact
            label="5-hour remaining"
            value={fiveHour?.remaining == null ? "—" : formatCredits(fiveHour.remaining)}
            detail={fiveHour?.limit == null ? "No current window meter" : `${formatCredits(fiveHour.used || 0)} used of ${formatCredits(fiveHour.limit)}`}
          />
        ) : null}
        {range === "weekly" ? (
          <SpendFact
            label="7-day remaining"
            value={weekly?.remaining == null ? "—" : formatCredits(weekly.remaining)}
            detail={weekly?.limit == null ? "No current window meter" : `${formatCredits(weekly.used || 0)} used of ${formatCredits(weekly.limit)}`}
          />
        ) : null}
        {range === "thirtyDay" ? (
          <SpendFact
            label="Official monthly used"
            value={monthly?.used == null ? "—" : formatCredits(monthly.used)}
            detail={monthly?.limit == null ? "Monthly account meter unavailable" : `${formatCredits(monthly.used || 0)} used of ${formatCredits(monthly.limit)} plan credits`}
          />
        ) : null}
        {range === "all" ? (
          <SpendFact
            label="Official remaining"
            value={balance?.value == null ? "—" : formatCredits(balance.value)}
            detail={balance?.detail || "Command Code account balance"}
          />
        ) : null}
        <SpendFact
          label="Local usage value"
        ...
        />
        <SpendFact
          label="GOAT credit eq."
          value={formatCredits(window.goatCredits, knownMinimum)}
          detail="Model allowance multiplier applied"
        />
        {(range === "fiveHour" || range === "weekly") ? (
          <SpendFact
            label="Unattributed"
            value={unattributed == null ? "—" : formatCredits(unattributed)}
            detail={unattributed == null
              ? "No directly comparable official window"
              : "Official used minus locally attributable credits"}
          />
        ) : null}
      </dl>
      ...
      {window.models.length ? (
                ...
                return (
                  <tr key={model.slug}>
                    <td>
                      ...
                      <small>
                        {model.goatMonthlyAllowance == null
                          ? model.slug
                          : `Allowance $${model.goatMonthlyAllowance} · ${formatMultiplier(model.goatMultiplier)}`}
                      </small>
                    </td>
                    <td>{model.pricedRequests ? formatUsd(model.usageValueUsd, partial) : "—"}</td>
                    <td>{model.pricedRequests ? formatCredits(model.goatCredits, partial) : "—"}</td>
        ...
        <EmptyState
          icon={<Coins size={20} />}
          title="No Command Code requests in this range"
      ...
      {spend.recentRequests.length ? (
        <div className="us-spend-recent">
          <div className="us-spend-recent-heading">
            <div>
              <strong>Recent Command Code requests</strong>
              <small>Dynamic Router ledger · refreshed immediately after a completed routed request</small>
            </div>
              ...
              <tbody>
                {spend.recentRequests.map((request, index) => (
                  <tr key={`${request.at}:${request.slug}:${index}`}>
                    ...
                    <td>
                      {request.usageValueUsd == null ? "—" : formatUsd(request.usageValueUsd, !request.complete)}
                      {request.goatCredits == null ? null : <small>{formatCredits(request.goatCredits, !request.complete)} GOAT eq.</small>}
                    </td>
      ...
      <p className="us-spend-note">
        Provider totals above are authoritative. Model rows and recent requests are generated dynamically from Router traffic; this is not a fixed model list.
        {knownMinimum ? " A ≥ value is a known minimum because Command Code can omit exact prompt/cache usage from the routed response even though Studio has its own internal cost." : ""}
        {window.retrospectiveRequests > 0 ? " Earlier Sep 6 rows were reconstructed from their recorded tokens and are labeled as such." : ""}
      </p>
    </section>
  );
}
...
function XkiroSpendPanel({
  ...
  return (
    <section className="panel-section us-commandcode-panel us-xkiro-panel" aria-label="Xkiro account usage">
      ...
      <div className="us-xkiro-account-meta" aria-label="Xkiro account extras">
        <span>Free tokens <strong>{freeTokens?.remaining == null ? "—" : compactNumber(freeTokens.remaining)}</strong></span>
        <span>Wallet <strong>{wallet?.value == null ? "—" : formatUsd(wallet.value)}</strong></span>
      </div>
      ...
      <dl className="us-spend-summary is-windowed">
        ...
        {range === "all" ? (
          <SpendFact
            ...
            value={exactNumber(window.requests)}
            detail="All retained Xkiro Router requests"
          />
        ) : null}
        <SpendFact
        ...
        {range !== "all" ? (
          <SpendFact
            ...
            detail={unattributed == null
              ? "No directly comparable official total"
...
function formatUsd(value: number, minimum = false): string {
  const amount = Number(value) || 0;
  const digits = amount > 0 && amount < 0.01 ? 4 : 2;
  return `${minimum ? "≥" : ""}$${amount.toFixed(digits)}`;
}

function formatCredits(value: number, minimum = false): string {
  const amount = Number(value) || 0;
  const digits = amount > 0 && amount < 0.01 ? 4 : 2;
  return `${minimum ? "≥" : ""}${amount.toFixed(digits)}`;
}
...
function formatMultiplier(value?: number): string {
  return Number.isFinite(value) ? `${Number(value).toFixed(2).replace(/\.00$/, "")}×` : "rate unavailable";
}
...
function formatRecentInput(request: {
  ...
  if (input != null && input > 0) {
    ...
    return `${compactNumber(input)} in · ${compactNumber(cached)} cached`;
  }
  if (request.estimatedInputTokens != null && request.estimatedInputTokens > 0) {
    return `provider omitted · router est. ${compactNumber(request.estimatedInputTokens)}`;
  }
