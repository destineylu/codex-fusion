...
interface ModelFamily {
  ...
  displayName: string;
  routes: RouterModel[];
  /** Canonical provider bucket used only by the Control Center presentation. */
  providerGroupId?: string;
}

interface PickerProviderGroup {
  id: string;
  displayName: string;
...
function providerGroupId(providerId: string, canonicalIds: Set<string>): string {
  for (const suffix of ["-messages", "-responses"]) {
    if (!providerId.endsWith(suffix)) continue;
    const base = providerId.slice(0, -suffix.length);
...
export function ModelsPage({ target, catalog, setup, usage, api, refreshing, dataReady, onRefresh, runAction, focusRequest }: ModelsPageProps) {
  ...
  const [verifyingCatalogModels, setVerifyingCatalogModels] = useState<Set<string>>(new Set());
  ...
  const canonicalProviderIds = useMemo(() => new Set([
    ...(setup?.providers ?? []).map((provider) => provider.id),
    ...(target?.providers ?? []).map((provider) => provider.id),
    "openai",
    "local",
  ]), [setup?.providers, target?.providers]);
  const providerGroups = useMemo<PickerProviderGroup[]>(() => {
    const buckets = new Map<string, RouterModel[]>();
    for (const model of allModels) {
      const groupId = providerGroupId(model.provider, canonicalProviderIds);
      buckets.set(groupId, [...(buckets.get(groupId) ?? []), model]);
    }
    const directoryOrder = new Map(directory.map((entry, index) => [entry.id, index]));
    return [...buckets.entries()]
      .map(([id, routes]) => {
        const entry = directoryById.get(id) ?? {
          id,
          displayName: providerDisplayName(id),
          models: routes.filter((model) => model.available !== false),
          knownModels: [],
        };
        return {
          id,
          ...
          entry,
          families: groupModelFamilies(routes).map((family) => ({
            ...family,
            id: `${id}::${family.id}`,
            providerGroupId: id,
          })),
      ...
      .sort((left, right) =>
        (directoryOrder.get(left.id) ?? Number.MAX_SAFE_INTEGER) - (directoryOrder.get(right.id) ?? Number.MAX_SAFE_INTEGER)
        || left.displayName.localeCompare(right.displayName));
  }, [allModels, canonicalProviderIds, directory, directoryById]);
  const families = useMemo<ModelFamily[]>(() => providerGroups.flatMap((group) => group.families), [providerGroups]);
  ...
  const providerNames = useMemo(() => {
    const names = new Map(directory.map((entry) => [entry.id, entry.displayName]));
    for (const group of providerGroups) {
      names.set(group.id, group.displayName);
      for (const family of group.families) {
        for (const model of family.routes) names.set(model.provider, group.displayName);
      }
    }
    return names;
  }, [directory, providerGroups]);
  // Provider grouping is the primary picker navigation; the optional filter
  // merely collapses the visible sections when the operator wants one account.
  const filterProviders = useMemo(() => providerGroups.map((group) => group.entry), [providerGroups]);
  ...
  // Add-model discovery is provider-scoped. Fetching every connected catalog
  // when the dialog opens is both slow and noisy on accounts with many routes;
  // selecting one provider first mirrors the credential boundary and makes an
  // explicit refresh spend network time only where the operator is browsing.
  const loadProviderCatalogs = async (providerId: string, { refresh = false } = {}) => {
    if (!api || loadingConnectedCatalogs) return;
    const entry = directoryById.get(providerId);
    if (!entry || !catalogEligible(entry) || !providerConnected(entry, enabledProviders)) return;
    const requests = (entry.setup?.catalogSources ?? [])
      .map((source) => ({ entry, sourceId: source.id }))
      .filter(({ sourceId }) => refresh || (catalogStates[sourceId]?.status ?? "idle") === "idle");
    if (!requests.length) return;
    setLoadingConnectedCatalogs(true);
    try {
      await Promise.all(requests.map(({ entry: provider, sourceId }) => discoverCatalog(provider, sourceId, { refresh })));
    } finally {
  ...
  const verifyAndAddCatalogModel = async (model: CatalogModel) => {
    if (!api || model.registered || model.addable) return;
    const entry = directoryById.get(model.providerId);
    if (!entry) return;
    setVerifyingCatalogModels((current) => new Set(current).add(model.key));
    let applied = false;
    try {
      await runAction(`Verify and add ${model.displayName}`, async () => {
        const report = await api.verifyProviderModel(model.sourceId, model.modelId);
        applied = Boolean(report.alreadyRegistered || report.applied?.entry || report.safeToCurate);
      });
      // verify-model refreshes the provider catalog before it spends quota.
      // Re-read the now-fresh stored list rather than billing a second network
      // discovery just to change this dialog's Added badge.
      if (applied) await discoverCatalog(entry, model.sourceId);
    } finally {
      setVerifyingCatalogModels((current) => {
  ...
  const renderConnections = () => !dataReady.providers && !setup ? (
    ...
    <ConnectionsBar
      ...
      onSignIn={(entry) => {
        ...
        if (entry.id === "antigravity-oauth" && !entry.setup.clientSecretConfigured) {
          setCredentialProvider({
            ...entry.setup,
            configured: false,
            credentialLabel: "OAuth client secret",
          });
          return;
        }
      ...
      <CredentialDialog
        ...
        onSave={(provider, secret) => {
          ...
          if (provider.id === "antigravity-oauth") {
            return runProviderCredentialAction(provider, `Save ${provider.displayName} OAuth client secret and sign in`, async () => {
              await api.saveProviderCredential(provider.id, secret);
              await api.connectProvider(provider.id);
            });
          }
          return runProviderCredentialAction(provider, `Save ${provider.displayName} credential`, () => api.saveProviderCredential(provider.id, secret));
        }}
  ...
  const filteredFamilies = useMemo(() => {
    ...
    return families.filter((family) => {
      if (activeProviderFilter !== "all" && family.providerGroupId !== activeProviderFilter) return false;
      if (!needle) return true;
      return `${family.displayName} ${family.routes.map((model) => `${model.displayName} ${model.slug} ${providerNames.get(model.provider) || providerDisplayName(model.provider)}`).join(" ")}`
        .toLowerCase()
        .includes(needle);
    });
  }, [activeProviderFilter, families, modelSearch, providerNames]);
  ...
  const visibleProviderGroups = useMemo(() => {
    const visibleByFamily = new Map(visibleRows.map((row) => [row.family.id, row]));
    return providerGroups
      .map((group) => ({
        ...group,
        rows: group.families
          .map((family) => visibleByFamily.get(family.id))
  ...
  const renderRow = ({ family, usable, inPicker, on }: (typeof rows)[number]) => (
    <ModelFamilyRow
      ...
      onConnect={(providerId) => {
        const groupId = providerGroupId(providerId, canonicalProviderIds);
        const entry = directoryById.get(groupId) ?? directoryById.get(providerId);
        if (!entry?.setup) return;
        if (entry.id === "antigravity-oauth") {
          if (!api) return;
          if (!entry.setup.clientSecretConfigured) {
            setCredentialProvider({
              ...entry.setup,
              configured: false,
              credentialLabel: "OAuth client secret",
          ...
          void runProviderCredentialAction(
            entry.setup,
            `Start ${entry.displayName} sign-in`,
            () => api.connectProvider(entry.id),
          );
          return;
        }
        if (entry.setup.kind === "oauth" || entry.setup.signIn) openProviderMenu(entry.id);
  ...
  return (
    <>
      <div className="providers-models-page models-page">
        ...
        <section className="panel-section pm-model-catalog" id="model-catalog-controls">
          ...
          {!connectedProviderCount && !families.length ? (
            ...
            />
          ) : visibleProviderGroups.length || pendingSlugs.length ? (
            <>
              {pendingSlugs.length ? (
                <div className="pm-family-list"><PendingModelRows slugs={pendingSlugs} /></div>
              ) : null}
      ...
      <AddModelsDialog
        open={addModelsOpen}
        directory={directory.filter((entry) => providerConnected(entry, enabledProviders))}
        catalogStates={catalogStates}
        loading={loadingConnectedCatalogs}
        disabled={!api}
        pendingModels={pendingModels}
        verifyingModels={verifyingCatalogModels}
        onLoadProvider={(providerId, options) => void loadProviderCatalogs(providerId, options)}
        onAdd={(selection) => void addCatalogModels(selection)}
        onVerifyAndAdd={(model) => void verifyAndAddCatalogModel(model)}
        ...
        <>
          ...
          <div className="pm-connection-menu-actions">
            {setup.kind === "oauth" || setup.signIn ? (
              <Button
                variant="ghost"
                disabled={!apiAvailable || (platform !== "darwin" && entry.id !== "antigravity-oauth")}
                title={platform === "darwin" || entry.id === "antigravity-oauth"
                  ? undefined
...
function ModelFamilyRow({
  ...
  const providerLabels = [...new Set(family.routes.map((model) => providerNames.get(model.provider) || providerDisplayName(model.provider)))];
  ...
  const facts = [
    providerLabels.length > 1 ? `${providerLabels.length} providers` : providerLabels[0] || maker.name,
        ...
        <div className="pm-family-action">
          {blocked ? (
            <Button variant="secondary" disabled={!apiAvailable} onClick={() => onConnect(providerIds[0])}>
              {providerLabels.length > 1 ? "Connect a provider" : `Connect ${providerLabels[0] || providerIds[0]}`}
            </Button>
          ) : (
            <>
        ...
        {multiRoute ? (
          <>
            <div className="pm-family-route-note">
              This model has more than one route through this provider group. Each route keeps its own protocol and capability profile.
            </div>
...
// Provider-first curation keeps model discovery cheap and legible. The router
// can have dozens of connected catalogs, but the operator is normally adding
// from one account at a time; choose that boundary before spending a refresh.
function AddModelsDialog({
  ...
  onLoadProvider: (providerId: string, options?: { refresh?: boolean }) => void;
  onAdd: (selection: CatalogModel[]) => void;
  onVerifyAndAdd: (model: CatalogModel) => void;
  onClose: () => void;
}) {
  const eligibleProviders = useMemo(() => directory.filter(catalogEligible), [directory]);
  ...
  useEffect(() => {
    ...
    if (!eligibleProviders.some((entry) => entry.id === providerId)) {
      setProviderId(eligibleProviders[0]?.id ?? "");
    }
  }, [eligibleProviders, open, providerId]);

  const provider = eligibleProviders.find((entry) => entry.id === providerId);
  const providerDirectory = provider ? [provider] : [];
  const catalogModels = useMemo(
    (): CatalogModel[] => (open && provider ? loadedCatalogModels(providerDirectory, catalogStates) : []),
    [catalogStates, open, provider],
  );
  const matching = useMemo(() => {
    const needle = query.trim().toLowerCase();
    if (!needle) return catalogModels;
    return catalogModels.filter((model) => (
      `${model.displayName} ${model.modelId} ${model.sourceName}`.toLowerCase().includes(needle)
    ));
  }, [catalogModels, query]);
  const providerSourceIds = new Set(provider?.setup?.catalogSources?.map((source) => source.id) ?? []);
  const providerStates = Object.entries(catalogStates)
    .filter(([sourceId]) => providerSourceIds.has(sourceId))
    .map(([, state]) => state);
  // Freshness belongs to the selected provider, not to whichever other account
  // happened to refresh most recently.
  const lastRead = providerStates
    .map((state) => state.data?.fetchedAt)
    .filter((value): value is string => Boolean(value))
    .sort()
    .at(-1);
  const loaded = providerStates.some((state) => Boolean(state.data));
  const errors = providerStates.filter((state) => state.status === "error");
  const selectedKeys = new Set(selected);
  const selectedModels = catalogModels.filter((model) => selectedKeys.has(model.key));
  const shown = matching.slice(0, shownLimit);
  const providerCounts = new Map(eligibleProviders.map((entry) => [
    entry.id,
    loadedCatalogModels([entry], catalogStates).length,
  ]));
  ...
  return (
    <Dialog
      open={open}
      title="Add models"
      description="Choose a provider, refresh only that catalog, then add compatible models directly. Blocked models can be live-verified before they are added."
      onClose={onClose}
    >
      <div className="pm-add-models">
        <div className="pm-add-models-shell">
          <aside className="pm-add-models-providers" aria-label="Model providers">
            <div className="pm-add-models-providers-title">Providers</div>
            {eligibleProviders.map((entry) => {
              const count = providerCounts.get(entry.id) ?? 0;
              return (
                <button
                  type="button"
                  key={entry.id}
                  className="pm-add-models-provider-tab"
                  data-active={providerId === entry.id}
                  aria-pressed={providerId === entry.id}
                  onClick={() => selectProvider(entry.id)}
                >
                  <ProviderLogo providerId={entry.id} displayName={entry.displayName} size="small" />
                  <span><strong>{entry.displayName}</strong><small>{count ? `${count} models` : "Not loaded"}</small></span>
                </button>
              );
            })}
          </aside>

          <div className="pm-add-models-browser">
            {provider ? (
              <>
                <div className="pm-add-models-provider-head">
                  <div className="pm-add-models-provider-title">
                    <ProviderLogo providerId={provider.id} displayName={provider.displayName} size="medium" />
                    <div>
                      <strong>{provider.displayName}</strong>
                      <small>{provider.setup?.catalogSources?.map((source) => source.displayName).join(" · ") || provider.id}</small>
                    </div>
                  </div>
                  <Button
                    variant={loaded ? "ghost" : "primary"}
                    disabled={disabled || loading}
                    onClick={() => onLoadProvider(provider.id, { refresh: true })}
                  >
                    {loading ? "Fetching models" : loaded ? "Refresh model list" : "Fetch model list"}
                  </Button>
                </div>

                <div className="pm-add-models-toolbar">
                  <SearchField value={query} onChange={setQuery} placeholder={`Search ${provider.displayName}`} />
                  <span className="pm-results-count" aria-live="polite">
                    {loading && !loaded
                      ? "Loading catalog"
                      : loaded
                        ? `${matching.length} of ${catalogModels.length} models${lastRead ? ` · read ${formatDateTime(lastRead)}` : ""}`
                        : "Fetch the provider list to browse models"}
                  </span>
                </div>

                {loading && !catalogModels.length ? <CatalogSkeleton label={`Loading ${provider.displayName} catalog`} /> : null}

                {!loading && !loaded ? (
                  <EmptyState
                    icon={<Plus size={20} />}
                    title={`Fetch ${provider.displayName} models`}
                    body="The model list is loaded only for the provider you choose, so accounts with many connected routes stay fast."
                  />
                ) : null}

                {!loading && loaded && !catalogModels.length ? (
                  <EmptyState
                    icon={<SearchX size={20} />}
                    title={errors.length ? "Catalog could not be loaded" : "No models published"}
                    body={errors[0]?.error || `${provider.displayName} returned an empty model catalog.`}
                  />
                ) : null}

                {shown.length ? (
                  <div className="pm-add-models-list" role="list" aria-label={`${provider.displayName} catalog models`}>
                    {shown.map((model) => {
                      const adding = (pendingModels[model.providerId]?.[model.modelId] ?? 0) > 0;
                      const verifying = verifyingModels.has(model.key);
                      ...
                      return (
                        ...
                        >
                          <input
                            ...
                            checked={checked}
                            disabled={disabled || model.registered || blocked || adding || verifying
                          ...
                          <span className="pm-add-models-provider">{model.sourceName}</span>
                          ...
                          <div className="pm-add-models-action">
                            {adding ? <Badge tone="neutral">Adding</Badge>
                              : verifying ? <Badge tone="neutral">Verifying…</Badge>
                              : model.registered ? <Badge tone="neutral">Added</Badge>
                              : blocked ? (
                                <Button variant="secondary" disabled={disabled} onClick={() => onVerifyAndAdd(model)}>
                                  Verify & add
                                </Button>
                              ) : (
                                <Button
                                  ...
                                  disabled={disabled}
                                  onClick={() => {
                                    setSelected((current) => current.filter((key) => key !== model.key));
                                    onAdd([model]);
                                  }}
                                >
                                  <Plus aria-hidden size={13} strokeWidth={1.9} /> Add
                                </Button>
                              )}
                          </div>
                        </div>
                ...
                {catalogModels.length && !matching.length ? (
                  <div className="pm-add-models-empty">No {provider.displayName} model matches this search.</div>
                ) : null}
              </>
            ) : (
              <EmptyState icon={<Plus size={20} />} title="No provider catalogs" body="Connect a provider that exposes a model catalog first." />
            )}
          </div>
        </div>
        ...
        <div className="dialog-actions">
          <span className="pm-add-models-hint">
            Select one provider at a time. Add uses the catalog gate; Verify & add spends a small amount of provider quota and publishes only after compatibility passes.
          </span>
          <Button variant="secondary" onClick={onClose}>Close</Button>
          <Button
            variant="primary"
            disabled={disabled || !selectedModels.length}
            onClick={() => { onAdd(selectedModels); setSelected([]); }}
          >
            {selectedModels.length ? `Add ${selectedModels.length} selected` : "Add selected"}
          </Button>
        </div>
      </div>
    </Dialog>
...
function connectionMethod(entry: ProviderDirectoryEntry): string {
  ...
  if (entry.id === "antigravity-oauth") return "Browser OAuth";
...
function connectionDetail(entry: ProviderDirectoryEntry, accountStatus?: string, accountMessage?: string, canOpenTerminal?: boolean): string {
  ...
  if (entry.id === "antigravity-oauth") {
    if (!entry.setup.clientSecretConfigured) {
      return "Save the Google OAuth client secret once, then Control Center opens the router-managed browser sign-in. The owner-only secret is reused by the background service when tokens refresh.";
    }
    if (entry.setup.configured) return "OAuth session ready. Sign-in and token refresh use the router-managed browser flow; no provider CLI is required.";
    return "Open the router-managed Google browser sign-in. No provider CLI is required.";
  }
