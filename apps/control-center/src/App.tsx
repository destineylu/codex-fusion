...
import { applyUiScale, detectUiScale, storeUiScale, type UiScale } from "./ui-scale";
import {
...
export default function App() {
  ...
  const [uiScale, setUiScale] = useState<UiScale>(detectUiScale);
  ...
  const refreshUsage = useCallback(async () => {
    ...
    const accountUsage = async () => {
      try {
        return await api.getAccountUsage();
      } catch (error) {
        // Codex app-server usage reads occasionally miss their own 10-second
        // deadline even while the next identical read completes in a few
        // seconds. Retry that one transient shape once instead of stretching
        // the deadline for every poll or masking authentication failures.
        if (!isCodexAccountUsageTimeout(error)) throw error;
        return api.getAccountUsage();
      }
    };
    const results = await Promise.allSettled([
      settleRead("accountUsage", accountUsage(), setAccountUsage),
    ...
    if (failure) setLoadError(readableError(failure.reason));
      ...
      // A transient account-usage warning must not stay pinned forever after a
      // later background read succeeds. Leave unrelated core/provider errors
      // alone because this refresh did not prove those sources recovered.
      setLoadError((current) => current && current.startsWith("Codex account usage is temporarily unavailable")
        ? null
        : current);
    }
  }, [api, settleRead]);
  ...
  const page = (() => {
    ...
    switch (view) {
      ...
      case "settings": return <SettingsPage {...shared} health={health} presence={presence} chatgptSession={snapshot?.chatgptSession} theme={theme} onTheme={setTheme} language={language} onLanguage={setLanguage} uiScale={uiScale} onUiScale={setUiScale} t={t} />;
    }
  })();
...
function isCodexAccountUsageTimeout(error: unknown): boolean {
  return errorText(error).includes("Codex account usage request timed out.");
}

function readableError(error: unknown): string {
  if (isCodexAccountUsageTimeout(error)) {
    return "Codex account usage is temporarily unavailable. Router traffic is still available; refresh to retry.";
  }
