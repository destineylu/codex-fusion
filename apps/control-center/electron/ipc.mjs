...
const CODEX_AGENT_MODES = ["single", "team"];
const CODEX_AUTO_RESUME_ACTIONS = ["install", "doctor", "dry-run", "enable-autostart", "disable-autostart", "enable-reset-credit", "disable-reset-credit"];
const CODEX_CHATGPT_WEB_ACTIONS = ["install", "start-managed", "show-managed", "start-daemon", "stop-managed", "verify-isolation"];
// Agent Mode remains project-scoped, but the distribution must not embed one
// operator's project path. Set CODEX_ROUTER_AGENT_MODE_PROJECT_ROOT before
// launching Control Center to expose Single / Team for that project; otherwise
// the feature stays unavailable instead of guessing a filesystem location.
const CODEX_AGENT_MODE_PROJECT_ROOT = String(process.env.CODEX_ROUTER_AGENT_MODE_PROJECT_ROOT || "").trim();
...
// Blocked-model verification can probe more than one protocol before it runs
// the routed compatibility check. Its individual provider probes already have
// bounded timeouts, so the desktop wrapper must outlive their legitimate sum.
const MODEL_VERIFY_TIMEOUT_MS = 10 * 60_000;
...
function codexAgentModePaths() {
  if (!CODEX_AGENT_MODE_PROJECT_ROOT) return undefined;
  const projectRoot = path.resolve(CODEX_AGENT_MODE_PROJECT_ROOT);
  const codexDir = path.join(projectRoot, ".codex");
  return {
    projectRoot,
    codexDir,
    current: path.join(codexDir, "config.toml"),
    state: path.join(codexDir, "agent-mode.txt"),
    single: path.join(codexDir, "config.single.toml"),
    team: path.join(codexDir, "config.team.toml"),
  };
}

function getCodexAgentModeSnapshot() {
  const paths = codexAgentModePaths();
  if (!paths) {
    return { supported: false, mode: "unknown", why: "Set CODEX_ROUTER_AGENT_MODE_PROJECT_ROOT to a project containing .codex/config.single.toml and config.team.toml." };
  }
  if (!existsSync(paths.codexDir) || !existsSync(paths.single) || !existsSync(paths.team)) {
    return { supported: false, mode: "unknown", projectRoot: paths.projectRoot, why: "Codex Agent Mode templates are missing." };
  }
  if (!existsSync(paths.current) || !existsSync(paths.state)) {
    return { supported: true, mode: "unknown", projectRoot: paths.projectRoot, why: "Codex Agent Mode state is incomplete." };
  }
  const state = readFileSync(paths.state, "utf8").trim().toLowerCase();
  if (!CODEX_AGENT_MODES.includes(state)) {
    return { supported: true, mode: "unknown", projectRoot: paths.projectRoot, why: "agent-mode.txt is not single or team." };
  }
  const current = readFileSync(paths.current);
  const expected = readFileSync(state === "single" ? paths.single : paths.team);
  if (!current.equals(expected)) {
    return { supported: true, mode: "unknown", projectRoot: paths.projectRoot, why: `config.toml does not match config.${state}.toml.` };
  }
  return { supported: true, mode: state, projectRoot: paths.projectRoot };
}

function setCodexAgentMode(mode) {
  const value = oneOf(mode, CODEX_AGENT_MODES, "Codex Agent Mode");
  const paths = codexAgentModePaths();
  if (!paths || !existsSync(paths.codexDir)) throw new Error("Codex Agent Mode project is unavailable.");
  const source = value === "single" ? paths.single : paths.team;
  if (!existsSync(source)) throw new Error(`Codex Agent Mode template is missing: config.${value}.toml.`);
  writeFileSync(paths.current, readFileSync(source));
  writeFileSync(paths.state, value, "utf8");
  const result = getCodexAgentModeSnapshot();
  if (result.mode !== value) throw new Error("Codex Agent Mode changed but verification failed.");
  return result;
}
...
async function validateProvider(providerId, capability) {
  ...
  if (capability === "sign-in" && provider.kind !== "oauth") {
    throw new Error(`${provider.displayName || id} does not support interactive sign-in.`);
  }
  return { id, provider };
}
...
export function registerIpcHandlers({
  ...
  handle("getCodexSkillControl", async () => getCodexSkillControlSnapshot());
  ...
  handleAction("verifyProviderModel", async ({ providerId, modelId } = {}) => {
    const { id, provider } = await validateCatalogProvider(providerId);
    const model = stringValue(modelId, "Model id", MODEL_SLUG);
    // The button is the operator's explicit consent to the quota-consuming
    // probes. Keep the renderer on a fixed command shape: it can choose only a
    // validated provider-catalog id and one model id, never arbitrary routes,
    // flags, paths, credentials, or subprocess arguments.
    const result = await runRouterScript(
      "verify-model.mjs",
      [id, model, "--live", "--yes", "--apply", "--json"],
      { timeoutMs: MODEL_VERIFY_TIMEOUT_MS, allowNonZero: true },
    );
    let report;
    try {
      report = JSON.parse(result.stdout.trim());
    } catch {
      throw new Error("Model verification returned invalid JSON.");
    }
    if (result.code !== 0 || (!report?.alreadyRegistered && report?.safeToCurate !== true)) {
      throw new Error(report?.reason || `${provider.displayName || id} could not verify ${model}.`);
    }
    return report;
  });
  handleAction("connectProvider", async ({ providerId } = {}) => {
    ...
    if (id === "antigravity-oauth") {
      // Antigravity is router-owned browser OAuth, not a vendor CLI. Keep the
      // PKCE/callback/token work in the trusted router child, but let Electron
      // open the authorization URL. PowerShell Start-Process from a Windows
      // Limited-token child can succeed without surfacing a browser window.
      if (!shell?.openExternal) throw new Error("System browser sign-in is unavailable.");
      let output = "";
      ...
      await runControl(["login", id], {
        timeoutMs: 10 * 60_000,
        environmentOverrides: { CODEX_ROUTER_BROWSER_OPEN: "0" },
        onStdout: (chunk) => {
          if (browserOpened) return undefined;
          output = `${output}${chunk}`.slice(-32_768);
          if (!authorizationHref) {
            const match = output.match(/https:\/\/accounts\.google\.com\/o\/oauth2\/v2\/auth\?[^\s]+/);
            if (match) {
              let authorizationUrl;
              try { authorizationUrl = new URL(match[0]); }
              catch { throw new Error("Antigravity returned an invalid authorization URL."); }
              if (
                authorizationUrl.protocol !== "https:"
                || authorizationUrl.hostname !== "accounts.google.com"
                || authorizationUrl.pathname !== "/o/oauth2/v2/auth"
              ) throw new Error("Antigravity returned an unexpected authorization destination.");
              authorizationHref = authorizationUrl.href;
            }
          }
          if (output.includes("Antigravity OAuth callback ready.")) callbackReady = true;
          if (!authorizationHref || !callbackReady) return undefined;
          browserOpened = true;
          return shell.openExternal(authorizationHref);
        },
      });
      if (!browserOpened) throw new Error("Antigravity sign-in did not provide a browser authorization URL.");
      await runControl(["catalog-cache", "invalidate", id]);
      return { providerId: id, pending: false, browser: true };
    }
  ...
  handleAction("saveProviderCredential", async ({ providerId, credential } = {}) => {
    const rawId = stringValue(providerId, "Provider", PROVIDER_ID);
    if (typeof credential !== "string" || !credential.trim() || credential.length > 16 * 1024) {
      throw new Error("Credential is invalid.");
    }
    if (rawId === "antigravity-oauth") {
      const { id } = await validateProvider(rawId, "sign-in");
      await runRouterScript("antigravity-client-secret.mjs", ["set"], {
        stdin: credential,
        timeoutMs: 30_000,
      });
      return { providerId: id, clientSecretConfigured: true, persisted: true };
    }
    const { id } = await validateProvider(rawId, "credential");
  ...
  handleAction("setContextEconomy", async ({ enabled } = {}) => {
    if (typeof enabled !== "boolean") throw new Error("enabled must be boolean.");
    return runJson(["context-economy", enabled ? "on" : "off"], {
      timeoutMs: CATALOG_MUTATION_TIMEOUT_MS,
    });
  });
  ...
  handleAction("setCodexAgentMode", async ({ mode } = {}) => {
    return setCodexAgentMode(mode);
  }, { requiresCompatibleRouter: false });
  ...
  handleAction("setCodexContextMode", async ({ mode } = {}) => {
    ...
    return setCodexContextMode(mode);
  }, { requiresCompatibleRouter: false });
  handleAction("setCodexSkillException", async ({ skillName, enabled } = {}) => {
    if (typeof enabled !== "boolean") throw new Error("Skill enabled state must be boolean.");
    return setCodexSkillException(skillName, enabled);
  }, { requiresCompatibleRouter: false });
