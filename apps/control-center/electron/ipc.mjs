...
// Local operator patch: Control Center manages the verified project-scoped
// templates used by set-agent-mode.ps1. An environment override keeps the
// IPC reusable without letting the renderer choose arbitrary filesystem paths.
const CODEX_AGENT_MODE_PROJECT_ROOT = process.env.CODEX_ROUTER_AGENT_MODE_PROJECT_ROOT
  || (process.platform === "win32" ? "F:\\程序\\office-leasing-ai" : "");
...
function getCodexAgentModeSnapshot() {
  ...
  if (!paths) {
    return { supported: false, mode: "unknown", why: "Codex Agent Mode is configured only for the local Windows project." };
  }
