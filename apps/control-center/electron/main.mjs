...
import { controlCodexChatGptWeb, getCodexChatGptWebSnapshot } from "./codex-chatgpt-web.mjs";
import {
...
const CHATGPT_WEB_AUTOSTART_DELAY_MS = 30_000;
const CHATGPT_WEB_AUTOSTART_ATTEMPTS = 3;

function sleep(ms) {
  return new Promise((resolve) => setTimeout(resolve, ms));
}

async function recoverManagedChatGptWebAfterLogin() {
  for (let attempt = 1; attempt <= CHATGPT_WEB_AUTOSTART_ATTEMPTS; attempt += 1) {
    try {
      let snapshot = await getCodexChatGptWebSnapshot();
      if (!snapshot.installed) return;
      if (snapshot.routeOwner !== "router") {
        console.error(`ChatGPT Web autostart skipped: Router is not route owner (owner=${snapshot.routeOwner}).`);
        return;
      }
      if (snapshot.safeToDiscover && snapshot.bridgeReachable) return;

      if (!snapshot.running) {
        await controlCodexChatGptWeb("start-managed");
      }
      ...
      snapshot = await getCodexChatGptWebSnapshot();
      if (snapshot.routeOwner !== "router") return;
      await controlCodexChatGptWeb("start-daemon");
      ...
      throw new Error("managed bridge did not become reachable");
    } catch (error) {
      console.error(`ChatGPT Web managed autostart attempt ${attempt} failed: ${error?.message || error}`);
      if (attempt < CHATGPT_WEB_AUTOSTART_ATTEMPTS) await sleep(10_000);
    }
  }
}

function scheduleManagedChatGptWebAutostart() {
  if (process.platform !== "win32" || RENDERER.development || !trayOnlyInvocation) return;
  const timer = setTimeout(() => {
    void recoverManagedChatGptWebAfterLogin();
  }, CHATGPT_WEB_AUTOSTART_DELAY_MS);
  timer.unref?.();
}
...
if (primaryInstance && !quitForUpdateInvocation) {
  ...
  app.whenReady().then(() => {
    ...
    createWindow();
    scheduleManagedChatGptWebAutostart();
