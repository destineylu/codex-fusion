...
function runEntrypoint(entry, args = [], {
  ...
  if (onStdout !== undefined && typeof onStdout !== "function") {
    throw new TypeError("Router command stdout observer must be a function.");
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    child.stdout.on("data", (chunk) => {
      ...
      if (onStdout) {
        try {
          const observed = onStdout(text);
          if (observed && typeof observed.then === "function") {
            void observed.catch((error) => {
              void abortCommand(new Error(safeFailure(error?.message || "Router command stdout observer failed.")));
            });
          }
        } catch (error) {
          void abortCommand(new Error(safeFailure(error?.message || "Router command stdout observer failed.")));
